...
class ContentSystemBarClone:
	...
	def SetAnimationCursor(self):
		if self.GetTimelinePath(1) is not None:
			if self.ownerComp.op("TimelineHeader/CursorToSet").numChans > 0:
				frameToSet=self.ownerComp.op("TimelineHeader/CursorToSet")[0]
...
class ContentSystemBar:
	...
	def Update(self):
		...
		for mymodule in re.split(r' ', str(self.activeModulesFromContentSystem)):
			ui.status=mymodule
			moduleOp=op(parent.ssui.par.Contentsystempath).op(mymodule)
	...
	def GetModuleById(self, myid):
		...
		moduleOp=op(parent.ssui.par.Contentsystempath).op(moduleName)
