...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [OAK Device CHOP](<./OAK_Device_CHOP.md> "OAK Device CHOP").   
  
## Members`masterFrame`→`float`**(Read Only)** : 

> Get the master time index in frames.`masterSample`→`float`**(Read Only)** : 

> Get the master time index in samples.`masterSeconds`→`float`**(Read Only)** : 

> Get the master time index in seconds.`masterTimecode`→`str`**(Read Only)** : 

> Get the master time index as a timecode in the form 00:00:00.00.`runningFrame`→`float`**(Read Only)** : 

> Get or set the running time index in frames.`runningSample`→`float`**(Read Only)** : 

> Get or set the running time index in samples.`runningSeconds`→`float`**(Read Only)** : 

> Get or set the running time index in seconds.`runningTimecode`→`str`**(Read Only)** : 

> Get or set the running time index as a timecode in the form 00:00:00.00.`timecode`→`tdu.Timecode`**(Read Only)** : 

> Get the master timecode.

## Methods`lockDevice()`→`'depthai.Device'`: 
...
>     
[/code]`sendMessage(message, streamName: str)`→`None`: 
...
[code] 
    ...
    def onReady(oakDeviceOp):
    	# We get the depthai.Device object as`device`# https://docs.luxonis.com/projects/api/en/latest/components/device/	
    	# if device := oakDeviceOp.lockDevice():
