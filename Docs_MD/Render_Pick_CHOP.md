...
The Render Pick CHOP samples a rendering (from a [Render TOP](<./Render_TOP.md> "Render TOP") or a [Render Pass TOP](<./Render_Pass_TOP.md> "Render Pass TOP")) and returns 3D information from the geometry at that particular pick location. Values sampled can include position, normals, point color, texture coordinates, depth and the object's path. An [Info DAT](<./Info_DAT.md> "Info DAT") should be used to retrieve the object's path. 

The pick location is specified through uv coordinates of the rendering. These uv coordinates can be selected by clicking on a [Panel Component](<./Panel_Component.md> "Panel Component") or explicitly setting them in the U and V parameters in the Render Pick CHOP. 
...
See also the multi-sample [Render Pick DAT](<./Render_Pick_DAT.md> "Render Pick DAT"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[renderpickCHOP_Class](<./RenderpickCHOP_Class.md> "RenderpickCHOP Class")
...
Use Pickable Flags`usepickableflags`\- When turned on only geometry whose [Pickable Flag](<./Flag.md> "Flag") is on can be selected by the Render Pick CHOP. The [Pickable Flag](<./Pickable_Flag.md> "Pickable Flag") is found on all [Object](<./Object.md> "Object") components. 
...
Fetch Object Path`path`\- Return the path to the object that is picked. This result requires and [Info DAT](<./Info_DAT.md> "Info DAT") with its **Node Path** parameter referecning the Render Pick CHOP. 

Fetch Depth`depth`\- Returns the depth of the point picked on the geometry. This value a non-linear ratio of the point's position between the near and far planes of the [Depth Buffer](</index.php?title=Depth_Buffer&action=edit&redlink=1> "Depth Buffer \(page does not exist\)"). Channel is _depth_. 
...
Extra Information for the Render Pick CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
