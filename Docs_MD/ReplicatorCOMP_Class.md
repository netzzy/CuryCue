...
This class inherits from the [ COMP class](<./COMP_Class.md> "COMP Class"). It references a specific [Replicator COMP](<./Replicator_COMP.md> "Replicator COMP").   
  
## Members`curItem`→`OP`**(Read Only)** : 

> Reference to the current [operator](<./Operator.md> "Operator") replicant.
...
> The replicatorCOMP that created this operator, if any.`storage`→`dict`**(Read Only)** : 
