...
The Line Thick POP is the engine that generates polygons for the [Line MAT](<./Line_MAT.md> "Line MAT") before it's rendered. We're not sure how useful it is yet, but Vincent Houze thought we'd make a POP of it and see what happens. It takes line strips or points plus a [Camera COMP](<./Camera_COMP.md> "Camera COMP") and generates triangles for the camera-facing distance-dependent thickened, colored lines, the rounded corners at each junction, the optional circles at each point, and vectors that you can create in the Line MAT. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[linethickPOP_Class](</LinethickPOP_Class> "LinethickPOP Class")
...
Extra Information for the Line Thick POP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
