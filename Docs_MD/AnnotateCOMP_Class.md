...
This class inherits from the [ COMP class](<./COMP_Class.md> "COMP Class"). It references a specific [Annotate COMP](<./Annotate_COMP.md> "Annotate COMP"). 

## Members`enclosedOPs`→`List[OP]`**(Read Only)** : 

> Returns list of any visible operators enclosed by this annotation.`height`→`float`**(Read Only)** : 

> The annotation's height, as measured in network editor units.`utility`→`bool`: 
...
> 
> Can be overridden by`COMP.op(includeUtility=False)`,`COMP.ops(includeUtility=False)`,`COMP.findChildren(includeUtilty=False)`as well as the [OP Find DAT](<./OP_Find_DAT.md> "OP Find DAT") filter: "Include Utility".`width`→`float`**(Read Only)** : 

> The annotation's width, as measured in network editor units.`x`→`float`**(Read Only)** : 

> The annotation's left edge, as measured in network editor units.`y`→`float`**(Read Only)** : 
