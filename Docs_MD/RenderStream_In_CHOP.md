...
See the [RenderStream](<./RenderStream.md> "RenderStream") article for more information. 

See also [RenderStream Out TOP](<./RenderStream_Out_TOP.md> "RenderStream Out TOP"), [RenderStream In TOP](<./RenderStream_In_TOP.md> "RenderStream In TOP"), [RenderStream](<./RenderStream.md> "RenderStream"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[renderstreaminCHOP_Class](<./RenderstreaminCHOP_Class.md> "RenderstreaminCHOP Class")
...
Schema DAT`schemadat`\- This should be a DAT with a table containing the schema information to create sequencable parameters in Disguise. The Schema DAT supports the columns:`group, label, name, type, default, min, max, step`, where`group, label, name, type`are required. The`type`column should be one of:`float, image, text`. Float data will come in via the primary channels in this node. Image data can be obtained using a [RenderStream In TOP](<./RenderStream_In_TOP.md> "RenderStream In TOP"). Text data can be obtained using an [Info DAT](<./Info_DAT.md> "Info DAT") pointing to this node. 
