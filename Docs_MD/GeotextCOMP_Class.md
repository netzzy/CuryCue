...
This class inherits from the [ COMP class](<./COMP_Class.md> "COMP Class"). It references a specific [Geo Text COMP](<./Geo_Text_COMP.md> "Geo Text COMP"). 

## Members`textHeight`→`float`**(Read Only)** : 

> The height of the text excluding padding. Only valid in Text mode.`textWidth`→`float`**(Read Only)** : 

> The width of the text excluding padding. Only valid in Text mode

## Methods`evalTextSize(str)`→`Tuple[float, float]`: 
...
> Returns a list of [TextLine](<./TextLine_Class.md> "TextLine Class") objects for every visible character in the given text. The origin member of the TextLine will be the character's 2D layout position including all kerning, wrapping and alignment. This data is often used in a Spec DAT/CHOP for the Geo Text COMP after further transformations/projections are applied to the character position. With this technique, text can be projected onto arbitrary surfaces while still maintaining proper layout positioning. 
> 
