...
Any OP that has a`.timecode`member will also have a Timecode Info Type available to view the timecode value as a set of CHOP channels. See [OPs with a timecode member](<./Timecode.htm#Operators_with_a_.timecode_Member> "Timecode") and the [Timecode](<./Timecode.md> "Timecode") page for more info. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[infoCHOP_Class](<./InfoCHOP_Class.md> "InfoCHOP Class")
...
Scope`iscope`\- The names of the attributes to keep. Name patterns may be used as detailed in [Pattern Matching](<./Pattern_Matching.md> "Pattern Matching"). 
...
Children Cook Time`childcooktime`\- When the Info CHOP is monitoring a [Component](<./Component.md> "Component"), this toggle will enable a channel called`children_cpu_cook_time`which is the total cooktime of all the component's children. This is off by default, enabling it for a large network may impact performance as all cooktimes need to be summed. 
...
Extra Information for the Info CHOP can be accessed via an Info CHOP. __
