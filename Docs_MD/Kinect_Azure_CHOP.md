...
The Kinect Azure CHOP can be used to obtain body tracking information, including joint positions and rotations, and IMU sensor data from a Microsoft Kinect Azure camera or a Kinect compatible [Orbbec](<./Orbbec.md> "Orbbec") Camera (Femto Mega, Femto Bolt, etc). 

This CHOP requires a [Kinect Azure TOP](<./Kinect_Azure_TOP.md> "Kinect Azure TOP") to connect to the camera and to configure the camera's settings. Not all configuration settings support body tracking e.g. Passive IR depth mode. Body tracking can only be used by one hardware type at a time (Microsoft or Orbbec). If more than one hardware type tries to use body tracking in a project, only the first node to be cooked will be enabled. 

Use the Max Players parameter to determine how many players you would like the camera to track. The TOP will automatically assign any bodies that are discovered by the device to the available player spaces. Due to the system used to detect player skeletons from the depth camera image, body tracking data can lag multiple frames behind the image data. To more closely synchronize body and image data, use the 'Sync Body Tracking To Image' option on the primary [Kinect Azure TOP](<./Kinect_Azure_TOP.md> "Kinect Azure TOP") that is controlling the device (synchronization will increase lag in the image capture). 
...
See also [Kinect Azure TOP](<./Kinect_Azure_TOP.md> "Kinect Azure TOP"), [Kinect Azure Select TOP](<./Kinect_Azure_Select_TOP.md> "Kinect Azure Select TOP"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[kinectazureCHOP_Class](<./KinectazureCHOP_Class.md> "KinectazureCHOP Class")

## 

Parameters - Kinect Azure Page

Active`active`\- Enable to disable the capturing of body tracking data. The primary [Kinect Azure TOP](<./Kinect_Azure_TOP.md> "Kinect Azure TOP") must also be active to receive data. 

Kinect Azure TOP`top`\- The name of the [Kinect Azure TOP](<./Kinect_Azure_TOP.md> "Kinect Azure TOP") that is connected to the camera. 
...
Extra Information for the Kinect Azure CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
