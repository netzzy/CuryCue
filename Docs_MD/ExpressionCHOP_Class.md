...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [Expression CHOP](<./Expression_CHOP.md> "Expression CHOP"). 

## Members`chanIndex`→ **(Read Only)** : 

>`me.chanIndex`can be used in any parameter to give a different value for each [channel](<./Channel_Class.md> "Channel Class") being generated, for example`[3, 4, 5][me.chanIndex]`.`inputVal`→ **(Read Only)** : 

> The current value of the input sample being evaluated. To access channels from other inputs use the [operator's inputs](<./OP_Class.htm#Connection> "OP Class"). Example: me.inputs[1]['chan4'] will access chan4 of the second input.`sampleIndex`→ **(Read Only)** : 
