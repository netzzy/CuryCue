...
This class inherits from the [ DAT class](<./DAT_Class.md> "DAT Class"). It references a specific [Insert DAT](<./Insert_DAT.md> "Insert DAT"). 

## Members`subRow`→`int`**(Read Only)** : 

> Current row index for Insert DAT Table expressions.`subCol`→`int`**(Read Only)** : 

> Current col index for Insert DAT Table expressions.`fillName`→`str`**(Read Only)** : 
