...
The Ouster Select TOP can be used to create additional output images from sensor data collected by an [Ouster TOP](<./Ouster_TOP.md> "Ouster TOP"). Each output image contains 4 channels of 32bit floating point data encoded into the red, green, blue and alpha channels of the image. Available sensor data channels include: Range, Intensity, Reflectivity and Noise as well as 3D XYZ position values. Raw range data is measured in millimeters, while XYZ positions are calculated using a table of beam azimuth and altitude angles and then converted to meters. 

See also: [Ouster TOP](<./Ouster_TOP.md> "Ouster TOP")

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[ousterselectTOP_Class](</OusterselectTOP_Class> "OusterselectTOP Class")
...
Extra Information for the Ouster Select TOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
