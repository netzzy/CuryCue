...
This class inherits from the [ DAT class](<./DAT_Class.md> "DAT Class"). It references a specific [MQTT Client DAT](<./MQTT_Client_DAT.md> "MQTT Client DAT"). 

For information see [MQTT](<./MQTT.md> "MQTT"). 

## Members`isConnected`→`bool`**(Read Only)** : 

> Return True if connected to a broker, and False if not.

## Methods`publish(topic, payload, qos=0, retain=False)`→`None`: 
...
>     
[/code]`subscribe(topic, qos=0)`→`None`: 
