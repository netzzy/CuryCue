...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [DAT to CHOP](<./DAT_to_CHOP.md> "DAT to CHOP"). 

## Members`inputCell`→`Cell`**(Read Only)** : 

> The current input [cell](<./Cell_Class.md> "Cell Class") being evaluated. From the [cell](<./Cell_Class.md> "Cell Class") you can get its row, column and value. e.g. me.inputCell.val or use the specific members listed below.`inputCol`→`int`**(Read Only)** : 

> The current input colunn being evaluated.`inputRow`→`int`**(Read Only)** : 

> The current input row being evaluated.`inputTable`→`OP`**(Read Only)** : 

> The current input [DAT](<./DAT_Class.md> "DAT Class") being evaluated.
