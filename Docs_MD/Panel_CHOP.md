...
The Panel CHOP reads [Panel Values](<./Panel_Value.md> "Panel Value") from [Panel Components](<./Panel_Component.md> "Panel Component") into CHOP channels. Panel values can also be accessed by using the`panel`Member of the [PanelCOMP Class](<./PanelCOMP_Class.md> "PanelCOMP Class"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[panelCHOP_Class](<./PanelCHOP_Class.md> "PanelCHOP Class")
...
Queue Overlapping Events`queue`\- Queue all events that occur in a [time slice](<./Time_Slicing.md> "Time Slicing"). Some [panel values](<./Panel_Value.md> "Panel Value"), such as wheel or key, switches between a value and 0 in the same time slice and are considered instantaneous. When this parameter is off, only the last value in the time slice will be used, and changes could be missed. Turning this on will keep as many events as possible based on queue size. Each queued item will be released in the next time slice until the queue is empty, and value changes within the same time slice are synchronized. 
...
Extra Information for the Panel CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
