...
The Audio Movie CHOP plays the audio of a movie file that is played back with a [Movie File In TOP](<./Movie_File_In_TOP.md> "Movie File In TOP"). Use the Movie File In TOP parameter to specify which Movie File In TOP to get the audio signal from. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[audiomovieCHOP_Class](<./AudiomovieCHOP_Class.md> "AudiomovieCHOP Class")
...
Audio Sync Offset`syncoffset`\- Offsets the audio playback of the movie. This can be used to get better sync between the audio and images in the movie when there is audio latency in the system (For example, audio latency from the [Audio Device Out CHOP](<./Audio_Device_Out_CHOP.md> "Audio Device Out CHOP") queue). A negative value plays audio that amount of time sooner, to counteract delay introduced by buffering such as in [Audio Device Out CHOP](<./Audio_Device_Out_CHOP.md> "Audio Device Out CHOP"). 
...
Volume`volume`\- Set the level the signal retrieved from the movie. A setting of 1 is full signal while 0 is muted. 
...
Extra Information for the Audio Movie CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). 
