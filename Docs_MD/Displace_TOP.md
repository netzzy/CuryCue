...
**Displace Image** \- In using the Displace Image, for each pixel in the output, it gets the corresponding pixel from the input, and uses the red channel, green channel, and blue channel as its U, V, and W displacement. If the red, green, and blue are .5, .5, .5, then Uo = Ui, Vo = Vi, Wo = Wi. That is, there is no warp if the Displace Image is a 50% grey. Also the warp is reduced to 0 if the Displace Weight is 0. Where red < .5 in the Displace Image, it fetches a pixel from the left of Uo in the Source Image. If green < .5, it fetches a pixel from below Vo in the Source Image. If blue < .5, it fetches a pixel from slices in front of Wo in the Source Image where w = 0 is the frontmost slice. Thus the pixel is retrieved from Ui, Vi, Wi at Uo*Scale*(red-.5), Vo*Scale*(.5-green), Wo*Scale*(.5-blue) of the Source Image. 

**Uo, Vo, Wo offset** \- By default, the warping of each output pixel is relative to its Uo, Vo, Wo. But when the UV Weight parameter is 0, the displacement is relative to the center pixel of the Source Image. 

**Ua, Va, Wa anchor (offset) point** \- This zooms into a pixel of the input if you bring the two other weights down to 0. 

For the Displace Image, you can change which of its RGBA channels cause the warp. 

You can choose if there is wraparound in the image warping. If the computed Uo, Vo, Wo is less than 0 or greater than 1, it can wraparound, clamp or mirror. 

The Displace Image can be any photograph followed by a Slope TOP, which will give luminance gradients in red and green, with the neutral value at .5, exactly the form required by the Displace TOP. 
...
See also: [Remap TOP](<./Remap_TOP.md> "Remap TOP"), [Lookup TOP](<./Lookup_TOP.md> "Lookup TOP")

**Note:** This TOP supports 3D Textures and 2D Texture Arrays. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[displaceTOP_Class](<./DisplaceTOP_Class.md> "DisplaceTOP Class")

## 

Parameters - Displace Page

Resolution Source`resolutionsource`\- ⊞ \- This parameter determines the resolution of the output image unless manually overwritten on the [Common Page](<#Parameters_-_Common_Page>). 
* Source Image`source`\- The output image uses the same resolution as the Source image.
* Displace Image`displace`\- The output image uses the same resolution as the Displace Image.
...
Vertical Source`vertsource`\- ⊞ \- Instead of using the Green channel to displace vertically, you can choose a different channel. 
* Red`red`-
* Green`green`-
* Blue`blue`-
* Alpha`alpha`-
...
Z Source`zsource`\- ⊞ \- Instead of using the Blue channel to displace the z, you can choose a different channel. 
* Red`red`-
* Green`green`-
* Blue`blue`-
* Alpha`alpha`-
...
Source Midpoint`midpoint`\- ⊞ \- This value is the color values that will result in no displacement. Values below this will cause the displacement to come from the left/bottom/front of the pixel, while values above this will cause the displacement to come from the right/top/back of the pixel. 
...
Displace Weight`displaceweight`\- ⊞ \- This scales the offset caused by the Displace Image. It will cause the pixels fetched to be closer/farther along the sample vector created by the Horizontal, Vertical, and Z Source. 
...
Aspect Correct`aspectcorrect`\- When on, this will apply an aspect-correct to the displacement offset based on the source image's original aspect ratio. This is useful when working with ultrawide textures to prevent the displacement from stretching. 
...
Offset`offset`\- ⊞ \- The Offset is first multiplied by the Offset Weight. Then it will be added to the coordinates calculated after looking up into the displacement map. These final coordinates is what will be used to sample from the source image. 
...
Offset Weight`offsetweight`\- Scales the Offset parameter values. When this is 0 the Offset parameter will have no effect. 
...
Extra Information for the Displace TOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
