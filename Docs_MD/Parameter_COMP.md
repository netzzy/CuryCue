...
You can optionally have it display the header and the page names, and include built-in or [Custom Parameters](<./Custom_Parameters.md> "Custom Parameters"). 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[parameterCOMP_Class](<./ParameterCOMP_Class.md> "ParameterCOMP Class")
...
Input Editor`inputeditor`\- Include the multi-input editor UI. This appears at the end of multi-input parameter dialogs such as the one used in the [Math CHOP](<./Math_CHOP.md> "Math CHOP") or [Merge SOP](<./Merge_SOP.md> "Merge SOP"). 

Allow Expansion`allowexpand`\- Turn On/Off the ability for parameters to be expanded showing the [parameter mode](<./Parameter.htm#Parameter_Modes_and_Evaluation> "Parameter") and expression editing field. 

Built-In`builtin`\- Include the operator's built-in parameters. 

Custom`custom`\- Include the operator's [Custom Parameters](<./Custom_Parameters.md> "Custom Parameters"). 
...
Page Scope`pagescope`\- ⊞ \- Selects with pages of parameters to include. Use * for [Pattern Matching](<./Pattern_Matching.md> "Pattern Matching") all. 
* *`*`-
* Base`Base`-
...
Parameter Scope`parscope`\- ⊞ \- Selects which parameters to include. Use * for [Pattern Matching](<./Pattern_Matching.md> "Pattern Matching") all. 
* *`*`-
...
Extra Information for the Parameter COMP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). 
