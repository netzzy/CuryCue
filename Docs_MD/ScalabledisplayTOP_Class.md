...
This class inherits from the [ TOP class](<./TOP_Class.md> "TOP Class"). It references a specific [Scalable Display TOP](<./Scalable_Display_TOP.md> "Scalable Display TOP").   
  
## Members`cameraTransform`→`tdu.Matrix`**(Read Only)** : 

> Gets the loaded camera transform [matrix](<./Matrix_Class.md> "Matrix Class") for the configuration. This should be referenced in the 'Xform Matrix/CHOP/DAT' parameter of the [Camera COMP](<./Camera_COMP.md> "Camera COMP").`projection`→`tdu.Matrix`**(Read Only)** : 

> Gets the loaded projection [matrix](<./Matrix_Class.md> "Matrix Class") for the configuration. This should be referenced in the 'Proj Matrix/CHOP/DAT' parameter of the [Camera COMP](<./Camera_COMP.md> "Camera COMP"), with the 'Projection' set to 'Custom Projection Matrix'.`calibWidth`→`int`**(Read Only)** : 

> Gets the calibration width as defined by calibration file.`calibHeight`→`int`**(Read Only)** : 

> Gets the calibration height as defined by calibration file.`calibApproxWidth`→`int`**(Read Only)** : 

> Gets the calibration approximate width as defined by calibration file.`calibApproxHeight`→`int`**(Read Only)** : 

> Gets the calibration approximate height as defined by calibration file.
