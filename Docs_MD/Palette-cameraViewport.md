...
The cameraViewport component is a modified [Camera COMP](<./Camera_COMP.md> "Camera COMP") that can be used to interactively control a camera's position/rotation in a 3D scene. It can be used instead of the default Camera COMP. Its viewer is an interactive Panel containing a rendered image from any [Render TOP](<./Render_TOP.md> "Render TOP").`cameraViewport`has all the 3D interactions that TouchDesigner's 3D viewers have, as noted below. 
...
**Note** : Instead of the built-in [Panel Component](<./Panel_Component.md> "Panel Component") that provides an interactive rendered 3D viewer, you can specify your own Panel COMP, though it's usually un-necessary. 
...
**Note** : After referencing this camera in a [Render TOP](<./Render_TOP.md> "Render TOP"), it is important to also update the Render TOP parameter in`cameraViewport`to reference the same Render TOP. Unlike the regular Camera COMP, this camera component requires knowledge of the scene being rendered in order to calculate framing, pivot on geometry, and to update the viewer. 

For the viewer of the [Geometry COMP](<./Geometry_COMP.md> "Geometry COMP"), see [Geometry Viewer](<./Geometry_Viewer.md> "Geometry Viewer"). 

**Note** :`cameraViewport`is a replacement of the [arcBallCamera COMP](<./Palette-arcBallCamera.md> "Palette:arcBallCamera"). 
...
Frame Padding`Padding`\- Frame Padding controls how much padding there is between the bounds of the object and the closest edge of the frame when you hit home. By default it is 0.1 or 10% of the frame. It goes up to 0.5 when means 50% top and bottom or all padding. 
...
Home Angle`Homeangle`\- When you home (h) it lets you set the camera looking off the Z-axis. 
