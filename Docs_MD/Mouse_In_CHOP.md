...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[mouseinCHOP_Class](<./MouseinCHOP_Class.md> "MouseinCHOP Class")

## 

Parameters - Control Page

Active`active`\- ⊞ \- While **On** , the mouse movement will be output from and the CHOP will cook every frame. When set to **Off** it will not cook and the current mouse X or Y values will not be output. **While Playing** will capture mouse events only when the [Timeline](<./Timeline.md> "Timeline") is playing forward. 
* Off`off`-
* On`on`-
* While Playing`play`-
...
Extra Information for the Mouse In CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
