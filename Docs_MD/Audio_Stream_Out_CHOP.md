...
The Audio Stream Out CHOP can stream audio out to any [rtsp](<http://en.wikipedia.org/wiki/Real_Time_Streaming_Protocol>) client such as VideoLAN's VLC media player and Apple's Quicktime, or to a [WebRTC](<./WebRTC.md> "WebRTC") peer. 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[audiostreamoutCHOP_Class](<./AudiostreamoutCHOP_Class.md> "AudiostreamoutCHOP Class")
...
Mode`mode`\- ⊞ \- Select the stream out mode: either [RTSP](<./RTSP.md> "RTSP") or [WebRTC](<./WebRTC.md> "WebRTC"). 
* RTSP`rtsp`-
* WebRTC`webrtc`-
...
WebRTC`webrtc`\- Set the [WebRTC DAT](<./WebRTC_DAT.md> "WebRTC DAT") (ie. peer) to send the audio stream over. Setting this will automatically populate the WebRTC Connection parameter menu with available connections. 

WebRTC Connection`webrtcconnection`\- Select the [WebRTC](<./WebRTC.md> "WebRTC") peer-to-peer connection. Selecting this will automatically population the WebRTC Track parameter menu with available audio output tracks. 
