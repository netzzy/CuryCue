...
**Tip** : If you get audio popping or audio dropouts, it may be caused by your actual frame rate of your main`timeline`\- some frames may take too long to compute/render. See [Time Slicing](<./Time_Slicing.md> "Time Slicing"). There are four possible remedies: 
* The global CHOP [Maximum Time Slice Size](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)") may be shorter than your worst-case time steps. Its default is .2 seconds set in Edit -> Preferences -> CHOPs -> Maximum Time Slice Size). If any frame takes longer than the Maximum Time Slice Size time to draw, audio will pop.
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[audiodeviceoutCHOP_Class](<./AudiodeviceoutCHOP_Class.md> "AudiodeviceoutCHOP Class")
...
Cycle Channels in Output``\- When True the audio channels are sent to each audio output such that no outputs are left silent. For example this allows a single channel to be output on two stereo outputs. (ASIO only). 
