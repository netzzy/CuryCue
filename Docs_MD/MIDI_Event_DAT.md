...
The MIDI Event DAT logs all [MIDI](<./MIDI.md> "MIDI") messages coming into or out of TouchDesigner from all MIDI In/Out operators. Note: no messages will be logged if there are no active MIDI In or Out operators set to receive them. It outputs columns in a table format: message, type, channel, index, value. 
...
See also the [MIDI In DAT](<./MIDI_In_DAT.md> "MIDI In DAT"), [MIDI In Map CHOP](<./MIDI_In_Map_CHOP.md> "MIDI In Map CHOP"), [MIDI In CHOP](<./MIDI_In_CHOP.md> "MIDI In CHOP"), [MIDI Out CHOP](<./MIDI_Out_CHOP.md> "MIDI Out CHOP"), Tscript`midi()`Command. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[midieventDAT_Class](<./MidieventDAT_Class.md> "MidieventDAT Class")
...
Callbacks DAT`callbacks`\- Runs this script once for each row added to the table (ie. MIDI event recieved). See [midieventDAT_Class](<./MidieventDAT_Class.md> "MidieventDAT Class") for usage. 
...
Extra Information for the MIDI Event DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
