...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [File In CHOP](<./File_In_CHOP.md> "File In CHOP").   
  
## Members`chanIndex`→`int`**(Read Only)** : 

>`me.chanIndex`can be used in any parameter to give a different value for each [channel](<./Channel_Class.md> "Channel Class") being generated, for example`[3, 4, 5][me.chanIndex]`.`curVal`→`float`**(Read Only)** : 

> The current value of the sample being overridden.`sampleIndex`→`int`**(Read Only)** : 
