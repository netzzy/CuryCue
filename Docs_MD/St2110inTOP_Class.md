...
## Members`inputSignalFormat`→`str | None`**(Read Only)** : 

> If available for the current Library, returns a string for the input signal format. This string can be used to set the 'Signal Format' menu on the Video Device Out TOP.`isConnected`→`bool`**(Read Only)** : 
