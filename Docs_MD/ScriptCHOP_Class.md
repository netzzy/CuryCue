...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [Script CHOP](<./Script_CHOP.md> "Script CHOP"). 

## Members`timeSliceDefault`→`bool`**(Read Only)** : 

> Get the default [Time Slice](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)") for the [Script CHOP](<./Script_CHOP.md> "Script CHOP"). Equal to the first input's`isTimeSlice`.

## Methods

These methods can be used by including them in the onCook callback of the Script CHOP. Using them outside the callback may cause unexpected results.`copyNumpyArray(numpyArray, baseName='chan')`→`None`: 
...
> Remove all custom parameters from the Script CHOP. Returns number of destroyed pars.`sortCustomPages(*pages)`→`None`: 
...
> Remove all channels from the CHOP. The channel length, sample rate etc. remain unchanged.`appendCustomPage(name)`→`Page`: 
...
> Match all of this CHOPs channel data to the given [CHOP](<./CHOP.md> "CHOP"). This includes sample rate, length, channel names and channel data. 
> 
>   * chop - The CHOP to copy. This should be a [CHOP Class](<./CHOP_Class.md> "CHOP Class") instance, not a path to the CHOP.
...
> Get or set the number of samples (or indices) per channel. You can change the number of samples by setting this value, only in a scriptCHOP.`start`→`float`: 

> Get or set the start index of the channels. This can be modified only when the CHOP is a scriptCHOP.`end`→`float`: 

> Get or set the end index of the channels. This can be modified only when the CHOP is a scriptCHOP.`rate`→`float`: 

> Get or set the sample rate of the CHOP. This can be modified only when the CHOP is a scriptCHOP.`isTimeSlice`→`bool`: 

> Get or set the last cooked [Time Slice](<./Time_Slicing.md> "Time Slicing") value. True if the CHOP last cooked as a Time Slice. This can be modified only when the CHOP is a scriptCHOP`export`→`bool`: 
