...
The channels exported by the FreeD CHOP are: 
    ...
    _camera_id_ \- a number from 0 - 255 that can be used to store an id number for the camera
    ...
    _focus_ \- a 24-bit positive integer (0-16 million) in arbitrary units that relates to the focus ring on a camera lens. Applications can interpret this value according to the hardware in use.
    _zoom_ \- a 24-bit positive integer (0-16 million) in arbitrary units that relates to the zoom ring on a camera lens. Applications can interpret this value according to the hardware in use.
    _user_ \- a 16-bit positive integer (0 - 65k) that can be used by the camera system to send custom user-defined data
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[freedCHOP_Class](<./FreedCHOP_Class.md> "FreedCHOP Class")
