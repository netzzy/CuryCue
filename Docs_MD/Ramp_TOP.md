...
The data for each color keyframe in the ramp is held in the [DAT](<./DAT.md> "DAT") specified by the DAT parameter. Each row in this DAT (in [Table Format](<./Table_DAT.md> "Table DAT")) represents a color keyframe entry in the ramp. The first column is the color keyframe's position on the ramp, the range is 0-1. The next 4 columns are the RGBA value for the color keyframe at that position. The DAT can be edited directly and the ramp will update in real-time. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[rampTOP_Class](<./RampTOP_Class.md> "RampTOP Class")

## 

Parameters - Ramp Page

DAT`dat`\- Specifies the DAT which defines the entries in the ramp.`color`\- ⊞ \- The color and alpha of each ramp keyframe can be set here. Select between an HSV or RGB colorpicker, or click the "+" button to open a color dialog box with predefined colors. 
...
[Parameter Color Space Parameters](</index.php?title=Parameter_Color_Space_Parameters&action=edit&redlink=1> "Parameter Color Space Parameters \(page does not exist\)")
...
Extra Information for the Ramp TOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
