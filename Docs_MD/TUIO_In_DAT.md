...
The TUIO In DAT receives and parses [TUIO](<https://www.tuio.org/?tuio11>) messages (received over network) into columns in the table. TUIO packets OSC bundles, so TUIO data can also be viewed in its more raw form in an [OSC In DAT](<./OSC_In_DAT.md> "OSC In DAT"). It currently uses the TUIO 1.1 protocol. The TUIO 2.0 protocol is not yet supported. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[tuioinDAT_Class](</TuioinDAT_Class> "TuioinDAT Class")
...
Callbacks DAT`callbacks`\- The Callbacks DAT will get callbacks for TUIO events. See [tuioinDAT_Class](</TuioinDAT_Class> "TuioinDAT Class") for usage. 
...
Extra Information for the TUIO In DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). 
