...
This class inherits from the [ DAT class](<./DAT_Class.md> "DAT Class"). It references a specific [Media File Info DAT](<./Media_File_Info_DAT.md> "Media File Info DAT").   
  
## Members`acodec`→`str`**(Read Only)** : 

> Returns the audio codec of the media file.`acodecDesc`→`str`**(Read Only)** : 

> Returns a descriptive audio codec of the media file.`bitdepth`→`int`**(Read Only)** : 

> Returns the bit depth of the media file.`filename`→`str`**(Read Only)** : 

> Returns the name of the media file.`filepath`→`str`**(Read Only)** : 

> Returns the path of the media file.`fps`→`float`**(Read Only)** : 

> Returns the FPS of the media file.`frames`→`int`**(Read Only)** : 

> Returns the number of frames in the media file.`height`→`int`**(Read Only)** : 

> Returns the height of the media file.`metadata`→`dict`**(Read Only)** : 

> Returns dictionary containing all the metadata.`numChannels`→`int`**(Read Only)** : 

> Returns the number of audio channels in the media file.`numSamples`→`int`**(Read Only)** : 

> Returns the number of audio samples of the media file.`numSeconds`→`float`**(Read Only)** : 

> Returns the length of the media file in seconds.`pixel_aspect_ratio`→`float`**(Read Only)** : 

> Returns the pixel aspect ratio of the media file.`sampleRate`→`int`**(Read Only)** : 

> Returns the audio sample rate of the media file.`valuesAreValid`→`bool`**(Read Only)** : 

> Returns true if the values column have valid entries.`vcodec`→`str`**(Read Only)** : 

> Returns the video codec of the media file.`vcodecDesc`→`str`**(Read Only)** : 

> Returns a descriptive video codec of the media file.`width`→`int`**(Read Only)** : 

> Returns the width of the media file.

## Methods`fetchInfo(filename)`→`MediaFileInfo`: 
