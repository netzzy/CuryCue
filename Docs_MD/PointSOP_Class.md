...
## Members`inputColor`→`Tuple[float, float, float, float]`**(Read Only)** : 

> The current point color being evaluated, from the first input, or a default if not present, expressed as a 4-tuple (r, g, b, a).`inputColor2`→`Tuple[float, float, float, float]`**(Read Only)** : 

> The current point color being evaluated, from the second input, or a default if not present, expressed as a 4-tuple (r, g, b, a).`inputNormal`→`Tuple[float, float, float]`**(Read Only)** : 

> The current point normal being evaluated, from the first input, or a default if not present, expressed as a 3-tuple (n1, n2, n3).`inputNormal2`→`Tuple[float, float, float]`**(Read Only)** : 

> The current point normal being evaluated, from the second input, or a default if not present, expressed as a 3-tuple (n1, n2, n3).`inputPoint`→`Point`**(Read Only)** : 

> The current [point](<./Point_Class.md> "Point Class") being evaluated, from the first input.`inputPoint2`→`Point`**(Read Only)** : 

> The current [point](<./Point_Class.md> "Point Class") being evaluated, from the second input.`inputTexture`→`Tuple[float, float, float]`**(Read Only)** : 

> The current point texture being evaluated, from the first input, or a default if not present, expressed as a 3-tuple (u, v, w).`inputTexture2`→`Tuple[float, float, float]`**(Read Only)** : 
