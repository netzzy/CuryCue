...
## Members`numFields`→ **(Read Only)** : 

> Get the number of channels defined per point e.g. a point may have XYZ and/or RGB channels.`numPoints`→ **(Read Only)** : 

> Get the number of points available in the current image. This could be less than the number of points that were loaded if the image resolution has been reduced or is limited by the current license. This number may also be less than the number of pixels in the image if fewer points were loaded than the image can store.`numSrcPoints`→ **(Read Only)** : 

> Get the number of points in the original file. It is possible that the graphics card cannot create an image large enough to fit all of the available points in the file. In these cases the numSrcPoints will be greater than numLoadedPoints.`numLoadedPoints`→ **(Read Only)** : 
