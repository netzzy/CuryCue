...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [Clip Blender CHOP](<./Clip_Blender_CHOP.md> "Clip Blender CHOP").   
  
## Members`clipA`→`OP`**(Read Only)** : 

> The clip being executed or blended from.`clipB`→`OP`**(Read Only)** : 

> The clip being blended into.`currentClip`→`OP`**(Read Only)** : 

> Clip A before the start of a transition, clip B otherwise.`currentFrame`→`float`**(Read Only)** : 

> The frame index of the current clip output.`currentTime`→`float`**(Read Only)** : 

> The time value of the current clip output.`endBlendTime`→`float`**(Read Only)** : 

> The time at which the transition is complete.`isBlending`→`bool`**(Read Only)** : 

> True if the output is transitioning between clips.`isQueued`→`bool`**(Read Only)** : 

> True if transitions are delayed until current transition completes.`isTriggerWaiting`→`bool`**(Read Only)** : 

> True if there is a pending triggered transition.`lastClipA`→`float`**(Read Only)** : 

> Last value of clip A.`lastClipB`→`float`**(Read Only)** : 

> Last value of clip B.`nextClip`→`OP`**(Read Only)** : 

> The next clip to be sequenced.`startBlendTime`→`float`**(Read Only)** : 

> The time the transition will start.`triggerClip`→`OP`**(Read Only)** : 

> The clip .

## Methods`trigger(clipCHOP, waitEnd=False)`→`None`: 
