...
## Members`isConnected`→`bool`**(Read Only)** : 

> True if any device is currently streaming to this operator..`inputSignalFormat`→`str`**(Read Only)** : 

> If available for the current Library, returns a string for the input signal format. This string can be used to set the 'Signal Format' menu on the Video Device Out TOP.`timecode`→`tdu.Timecode`**(Read Only)** : 
