# Palette:synchroSDIIn

##   
  
Summary

This SDI In component is a wrapper for the videoDeviceIn TOP. It adds features for extracting timecode and conversion to a frame index as well as signal validation and a backup source for cleaner integration into user friendly systems. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[Palette:synchroSDIIn Ext](</index.php?title=Palette:synchroSDIIn_Ext&action=edit&redlink=1> "Palette:synchroSDIIn Ext \(page does not exist\)")

## 

Parameters - SDI In Page

Active`Active`\- When turned on the TOP captures the image stream from the selected device 

Library`Library`\- ⊞ \- Selects the library to use to interface with the video device or camera. 
...
Device`Device`\- ⊞ \- Selects which device to use. 
* NewTek NDI Video`V1`-
* Intel(R) RealSense(TM) Camera SR300 RGB`V1`-
* Decklink Video Capture (3)`V1`-
* Decklink Video Capture`V1`-
* OBS Virtual Camera`V1`-
* NotchCam`V1`-
* Decklink Video Capture (4)`V1`-
* Decklink Video Capture (2)`V1`-
...
Transfer Mode`Transfermode`\- ⊞ \- The mode used to transfer video over CPU and GPU memory. 
* Automatic`automatic`-
* Pre-Upload, Pinned`preuploadpinned`-
* Pre-Upload`preupload`-
* On-Demand, Pinned`ondemandpinned`-
* On-Demand`ondemand`-


Signal Resolution Backup`Signalresolutionbackup`\- ⊞ \- The resolution of the texture passed out when no valid input is detected. 
