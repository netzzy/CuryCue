...
**NOTE:** The Keyboard In CHOP does not handle rapid repeats of characters and does not output channels or all keys. A better way is as follows: Create a [Panel Execute DAT](<./Panel_Execute_DAT.md> "Panel Execute DAT"), and set the 'Panel' parameter to the panel in which keyboard keys need to be monitored. Set the Panel Value to:`key`. In this DAT, if you enter`print(panelValue)`in the onValueChange() callback, you will see a line printed for every keystroke. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[keyboardinCHOP_Class](<./KeyboardinCHOP_Class.md> "KeyboardinCHOP Class")

## 

Parameters - Keys Page

Active`active`\- ⊞ \- While **On** , the keyboard inputs will be monitored and the CHOP will cook every frame. When set to **Off** it will not cook and the current keyboard values will not be output. **While Playing** will capture keyboard events only when the [Timeline](<./Timeline.md> "Timeline") is playing forward. 
* Off`off`-
* On`on`-
* While Playing`play`-
...
Panels`panels`\- When a path to a [Panel COMP](<./Panel_Component.md> "Panel Component")(s) is specified, only keyboard events that take place when that panel(s) has focus will reported by the Keyboard In CHOP. 
...
Extra Information for the Keyboard In CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
