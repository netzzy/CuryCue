...
## Methods`appendPoint()`→`Point`: 

> Append a [point](<./Point_Class.md> "Point Class") to this SOP. The appended point will be returned.`appendPoly(numVertices, closed=True, addPoints=True)`→`Poly`: 

> Append a [poly](<./Poly_Class.md> "Poly Class") to this SOP. Returns the appended polygon. 
> 
>   * numVertices - Specifies the initial number of [vertices](<./Vertex_Class.md> "Vertex Class").
>   * closed - (Keyword, Optional) Specifies whether or not the last [vertex](<./Vertex_Class.md> "Vertex Class") of the polygon will connect to the first. An open polygon will be drawn as a line.
>   * addPoints - (Keyword, Optional) If True, a new [point](<./Point_Class.md> "Point Class") will be attached to each [vertex](<./Vertex_Class.md> "Vertex Class"), otherwise the [vertex](<./Vertex_Class.md> "Vertex Class") point references will need to be manually set afterwards. Use this option when creating [polygons](<./Poly_Class.md> "Poly Class") with shared [vertices](<./Vertex_Class.md> "Vertex Class").
>`appendBezier(numVertices, closed=False, order=4, addPoints=True)`→`Bezier`: 

> Append a [Bezier](<./Bezier_Class.md> "Bezier Class") to this SOP. Returns the appended Bezier. 
> 
>   * numVertices - Specifies the initial number of [vertices](<./Vertex_Class.md> "Vertex Class"). The number of [vertices](<./Vertex_Class.md> "Vertex Class") **must** correspond to the order (degree-1) and closed/open state of the curve. For closed curves, the number of vertices must be a multiple of the degree. For open curves, it must be one more than a multiple of the degree.
> 
...
[/code]
> 
>   * closed - (Keyword, Optional) Specifies whether or not the last [vertex](<./Vertex_Class.md> "Vertex Class") of the curve will connect to the first. An open Bezier will be drawn as a line.
>   * order - (Keyword, Optional) Specifies the degree of the Bezier. By default it creates cubic (order=4) Beziers.
>   * addPoints - (Keyword, Optional) If True, a new [point](<./Point_Class.md> "Point Class") will be attached to each [vertex](<./Vertex_Class.md> "Vertex Class"), otherwise the [vertex](<./Vertex_Class.md> "Vertex Class") point references will need to be manually set afterwards. Use this option when creating Beziers with shared vertices.
>`appendMesh(numRows, numCols, closedU=False, closedV=False, addPoints=True)`→`Mesh`: 

> Append a [mesh](<./Mesh_Class.md> "Mesh Class") to this SOP. Returns the appended mesh. 
> 
...
>   * addPoints - (Keyword, Optional) If True, a new [point](<./Point_Class.md> "Point Class") will be attached to each [vertex](<./Vertex_Class.md> "Vertex Class"), otherwise the [vertex](<./Vertex_Class.md> "Vertex Class") point references will need to be manually set afterwards. Use this option when creating [meshes](<./Mesh_Class.md> "Mesh Class") with shared [vertices](<./Vertex_Class.md> "Vertex Class").
>`clear()`→`None`: 

> Remove all geometry.`copy(sop)`→`None`: 
...
Geometry Attributes can be added to this operator using the`create(name, default)`method of the [Attributes Class](<./Attributes_Class.md> "Attributes Class")
[code] 
...
> Get or set [Compare Flag](<./Compare_Flag.md> "Compare Flag").`template`→`bool`: 

> Get or set [Template Flag](<./Template_Flag.md> "Template Flag").`points`→`td.Points`**(Read Only)** : 

> The set of [points](<./Points_Class.md> "Points Class") contained in this SOP.`prims`→`td.Prims`**(Read Only)** : 

> The set of [primitives](<./Prims_Class.md> "Prims Class") contained in this SOP.`numPoints`→`int`**(Read Only)** : 

> The number of [points](<./Points_Class.md> "Points Class") contained in this SOP.`numVertices`→`int`**(Read Only)** : 

> The number of [vertices](<./Vertex_Class.md> "Vertex Class") contained in all primitives within this SOP.`numPrims`→`int`**(Read Only)** : 

> The number of [primitivies](<./Prims_Class.md> "Prims Class") contained in this SOP.`pointAttribs`→`td.Attributes`**(Read Only)** : 

> The set of point [attributes](<./Attributes_Class.md> "Attributes Class") defined in this SOP.`primAttribs`→`td.Attributes`**(Read Only)** : 

> The set of primitive [attributes](<./Attributes_Class.md> "Attributes Class") defined in this SOP.`vertexAttribs`→`td.Attributes`**(Read Only)** : 

> The set of vertex [attributes](<./Attributes_Class.md> "Attributes Class") defined in this SOP.`pointGroups`→`dict`**(Read Only)** : 

> Returns a dictionary of point [groups](<./Group_Class.md> "Group Class") defined for this SOP.`primGroups`→`dict`**(Read Only)** : 

> Returns a dictionary of primitive [groups](<./Group_Class.md> "Group Class") defined for this SOP.`center`→`tdu.Position`: 

> Get or set the barycentric coordinate of this operator's geometry. It is expressed as a [Position](<./Position_Class.md> "Position Class").`min`→`tdu.Position`**(Read Only)** : 

> The minimum coordinates of this operator's geometry along each dimension, expressed as a [Position](<./Position_Class.md> "Position Class").`max`→`tdu.Position`**(Read Only)** : 

> The maximum coordinates of this operator's geometry along each dimension, expressed as [Position](<./Position_Class.md> "Position Class").`size`→`tdu.Position`**(Read Only)** : 

> The size of this operator's geometry along each dimension, expressed as a [Position](<./Position_Class.md> "Position Class").

## Methods`computeBounds()`→`Bounds`: 

> Returns an object with the bounds, center and size of the SOP's geometry. 
> 
>   * display - (Keyword, Optional) If set to True, only [SOP operators](<./SOP_Class.md> "SOP Class") whose[ display flag](<./Display_Flag.md> "Display Flag") set are included in the calculation. When False, the states of the display flag is ignored.
>   * render - (Keyword, Optional) If set to True, only [SOP operators](<./SOP_Class.md> "SOP Class") whose[ render flag](<./Render_Flag.md> "Render Flag") set are included in the calculation. When False, the state of the render flag is ignored.
...
> Unique id for the operator. This id can also be passed to the op() and ops() methods. Id's are not consistent when a file is re-opened, and will change if the OP is copied/pasted, changes OP types, deleted/undone. The id will not change if the OP is renamed though. Its data type is integer.`name`→`str`: 
...
> Returns the beginning portion of the name occurring before any digits.`passive`→`bool`**(Read Only)** : 

> If true, operator will not cook before its access methods are called. To use a passive version of an operator n, use passive(n).`curPar`→`td.Par`**(Read Only)** : 

> The parameter currently being evaluated. Can be used in a parameter expression to reference itself. An easy way to see this is to put the expression`curPar.name`in any string parameter.`curBlock`→ **(Read Only)** : 
...
> [Time Component](<./TimeCOMP_Class.md> "TimeCOMP Class") that defines the operator's time reference.`ext`→`class`**(Read Only)** : 

> The object to search for parent [extensions](<./Extensions.md> "Extensions"). 
[code]
>     me.ext.MyClass
>     
[/code]`fileFolder`→`str`**(Read Only)** : 
...
> Get a [module on demand](<./MOD_Class.md> "MOD Class") object that searches for DAT modules relative to this operator.`pages`→`list`**(Read Only)** : 

> A list of all built-in pages.`parGroup`→`tuple`**(Read Only)** : 
...
>     n.par['tx']
>     
[/code]`builtinPars`→`list or par`**(Read Only)** : 

> A list of all [built-in parameters](<./Par_Class.md> "Par Class").`customParGroups`→`list of parGroups`**(Read Only)** : 

> A list of all [ParGroups](<./ParGroup_Class.md> "ParGroup Class"), where a ParGroup is a set of parameters all drawn on the same line of a dialog, sharing the same label.`customPars`→`list of par`**(Read Only)** : 

> A list of all [custom parameters](<./Par_Class.md> "Par Class").`customPages`→`list`**(Read Only)** : 

> A list of all [custom pages](<./Page_Class.md> "Page Class").`replicator`→`OP or None`**(Read Only)** : 
...
> Returns the number of operators contained recursively within this operator. Only [component](<./COMP_Class.md> "COMP Class") operators have children.`op`→`OP or None`**(Read Only)** : 
...
[/code]
>> 
>> See also Parent Shortcut for more examples.`iop`→`OP`**(Read Only)** : 

> The Internal Operator Shortcut object, for accessing internal shortcuts. See also [Internal Operators](<./Internal_Operators.md> "Internal Operators"). **Note:** a version of this method that searches relative to the current operator is also in the global [td Module](<./Td_Module.md> "Td Module").`ipar`→`ParCollection`**(Read Only)** : 
...
>     
[/code]`enclosedBy`→`list`**(Read Only)** : 
...
> Get or set [Selected Flag](<./Selected_Flag.md> "Selected Flag"). This controls if the node is part of the network selection. (yellow box around it).`seq`→ **(Read Only)** : 
...
> Get or set the [operator](<./OP_Class.md> "OP Class") this operator is docked to. To clear docking, set this member to None.`docked`→`list`**(Read Only)** : 

> The (possibly empty) list of [operators](<./OP_Class.md> "OP Class") docked to this node.

### Connection

See also the`OP.parent`methods. To connect components together see [COMP_Class#Connection](<./COMP_Class.htm#Connection> "COMP Class") section.`inputs`→`list`**(Read Only)** : 

> List of input [operators](<./OP_Class.md> "OP Class") (via left side connectors) to this operator. To get the number of inputs, use len(OP.inputs).`outputs`→`list`**(Read Only)** : 

> List of output [operators](<./OP_Class.md> "OP Class") (via right side connectors) from this operator.`inputConnectors`→`list`**(Read Only)** : 

> List of input [connectors](<./Connector_Class.md> "Connector Class") (on the left side) associated with this operator.`outputConnectors`→`list`**(Read Only)** : 
...
> True if the operators is a [TOP](<./TOP.md> "TOP").`licenseType`→`str`**(Read Only)** : 
...
**NOTE** :`create()`,`copy()`and`copyOPs()`is done by the parent operator (a component). For more information see [COMP.create, COMP.copy and COMP.copyOPs methods](<./COMP_Class.htm#Methods> "COMP Class").`pars(pattern)`→`list`: 
...
> Returns a (possibly empty) list of operator dependency paths between this operator and the specified operator. Multiple paths may be found.`evalExpression(str)`→`value`: 
...
>     
[/code]`resetPars(parNames='*', parGroupNames='*', pageNames='*', includeBuiltin=True, includeCustom=True)`→`bool`: 
...
See the [Examine DAT](<./Examine_DAT.md> "Examine DAT") for procedurally viewing the contents of storage.`fetch(key, default, search=True, storeDefault=False)`→`value`: 
...
>     
[/code]`unstoreStartupValue(keys1, keys2..)`→`None`: 
