# Palette:superFormula

## 

Summary

The superFormula COMP creates a SOP using the 2D and 3D superformula based on equations proposed by Johan Gielis. More information about the superformula equation can be found [here](<https://en.wikipedia.org/wiki/Superformula>). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[Palette:superFormula Ext](</index.php?title=Palette:superFormula_Ext&action=edit&redlink=1> "Palette:superFormula Ext \(page does not exist\)")

## 

Parameters - Super Formula Page

Help`Help`\- Opens this page. 

Version`Version`\- Current version of this COMP. 

3D`Enable3d`\- Enables the 3D formula. 

Number of Points`Points`\- The number of points in the 2D shape. 

A1`A1`\- 

Rows`Rows`\- The number of rows and columns. Rows are horizontal lines; Columns are vertical lines. Two rows by two columns makes a square or rectangle. For example, one row and two columns makes a single line if Connectivity is set to Rows. For NURBS and Bezier surfaces, the Rows & Cols refer to the surface hull construction, and as such, the number of Rows and Cols should not be less than the Order in U or V. 

Columns`Cols`\- The number of rows and columns. Rows are horizontal lines; Columns are vertical lines. Two rows by two columns makes a square or rectangle. For example, one row and two columns makes a single line if Connectivity is set to Rows. For NURBS and Bezier surfaces, the Rows & Cols refer to the surface hull construction, and as such, the number of Rows and Cols should not be less than the Order in U or V. 
