...
The File Out POP allows you to write out POP contents to different file types. This includes point / geometry and scene file types. 

You can record a sequence of`.obj`or`.exr`files by setting the Type parameter to File Sequence. You can press Record, and then only start to create files when Pause is set to off, or record (create) a single file at a time with the Add Frame pulse parameter. 

It will try to output all the point, vertex and primitive attributes. Cases where it will not: 

Some file formats have fixed attributes that File Out POP look for such as the SPZ and OBJ file format. While other files allow arbitrary attribute writing such as the EXR and PLY file format. 

TBD: obj files: What is written to the file: What not written: 

FPS determines the number of files to output per second of the timeline. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[fileoutPOP_Class](<./FileoutPOP_Class.md> "FileoutPOP Class")
