...
The List Component lets you create large lists that are highly customizable via the List COMPs initialization and callback functions. Refer to the [Build a List COMP](<./Build_a_List_COMP.md> "Build a List COMP") article for more info on using this COMP. 

See Also: [Lister Custom COMP](<./Palette-lister.md> "Palette:lister"), a Python enhanced custom listCOMP. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[listCOMP_Class](<./ListCOMP_Class.md> "ListCOMP Class")

## 

Callbacks

Every cell, row, column and table in a List component is described by [attributes](<./ListAttribute_Class.md> "ListAttribute Class") which are specified through the initialize and callback functions of the List component. For usage please refer to the [![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>) [listCOMP_Class](<./ListCOMP_Class.md> "ListCOMP Class") page. 

## 

Drag/Drop

The listCOMP uses [Drag/Drop Callbacks](<./Drag-and-Drop.htm#Drag.2FDrop_Callbacks> "Drag-and-Drop") to implement dragging and dropping behavior. The provided`info`dictionary in those callbacks will include a`listCell`member describing what cell is being dragged or dropped on. A ListCell can also be in the`dragItems`list. 
...
Extra Information for the List COMP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
