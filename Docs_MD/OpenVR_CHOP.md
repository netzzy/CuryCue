...
The OpenVR CHOP receives positional data, frame rendering info, and action data from the [OpenVR](<./OpenVR.md> "OpenVR") SDK. Each CHOP can output in one of 5 modes: Sensors, Projection Matrices, Trackers, Frame Timings, Actions and Skeletons. 

### Actions

Actions are described in more details in [OpenVR Actions](</OpenVR_Actions> "OpenVR Actions"). 

### Getting Sensor Data at Higher Rates

By default when running a VR system the file will be throttled to the speed of the VR devices refresh rate by the OpenVR SDK. This helps ensure the low latency output required for a good VR experience. If only controllers/Vive trackers are being used for tracking in a non-VR situation, the file can run and sample those devices at a higher sample rate as long as no [OpenVR TOP](<./OpenVR_TOP.md> "OpenVR TOP") in the project. If an [OpenVR TOP](<./OpenVR_TOP.md> "OpenVR TOP") is present anywhere in the project, then playback will be throttled to the VR devices refresh rate. 

See also [OpenVR](<./OpenVR.md> "OpenVR"), [OpenVR TOP](<./OpenVR_TOP.md> "OpenVR TOP"), [OpenVR SOP](<./OpenVR_SOP.md> "OpenVR SOP"), [Audio Render CHOP](<./Audio_Render_CHOP.md> "Audio Render CHOP")

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[openvrCHOP_Class](<./OpenvrCHOP_Class.md> "OpenvrCHOP Class")
...
* Actions`actions`\- The action channels as defined by the [OpenVR Actions](</OpenVR_Actions> "OpenVR Actions") in used by the system.
...
Custom Actions`customactions`\- Turn on to allow specifying a custom [OpenVR Actions](</OpenVR_Actions> "OpenVR Actions") manifest file. 

Action Manifest`actionmanifest`\- A path to a [OpenVR Actions](</OpenVR_Actions> "OpenVR Actions") manifest file. By default this is using the same manifest OpenVR uses when Custom Actions is disabled. 
...
Extra Information for the OpenVR CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
