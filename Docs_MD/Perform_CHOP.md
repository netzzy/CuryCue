...
It outputs the "cook time" of the prior frame that was drawn, which is a reflection of how many frames TouchDesigner cooked in the prior second. The`cook`**Tip** : Put a Trail CHOP after the Perform CHOP to watch its progress. 
...
The Built-in Variables found on Dialogs -> Variables -> Built-in include more useful data on your running system. See also the Built-in Variables section of the [Variables](<./Variables.md> "Variables") page. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[performCHOP_Class](<./PerformCHOP_Class.md> "PerformCHOP Class")
...
Total Deactivated OP Calls`deactivatedops`\- Number of calls to cook a component that has its [Cooking Flag](<./Cooking_Flag.md> "Cooking Flag") turned off. 

Total OPs`totalops`\- Total number of OPs in the [.toe](<./.md> ".toe") file. 
...
Time Slice Step`timeslicestep`\- The number of frames that TouchDesigner stepped forward for the current cook. It's the length of the [Time Slice](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)") in frames. It will be equal to 1 when the system is taking`1000/root.time.rate`msec or less to complete one frame (16.666 msec for a rate of 60). 

Time Slice Milliseconds`timeslicemsec`\- The length of the current [Time Slice](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)") in milliseconds. 

Perform Mode`performmode`\- Monitors the state of [Perform Mode](<./Perform_Mode.md> "Perform Mode"). 
...
Optimized Expressions`optimizedexpression`\- The number of python expression that have been optimized, see [Optimized Python Expressions](<./Optimized_Python_Expressions.md> "Optimized Python Expressions"). 

Expressions Using Cache`cachedexpressions`\- The number of python expression that have been cached, see [Optimized Python Expressions](<./Optimized_Python_Expressions.md> "Optimized Python Expressions"). 
...
Extra Information for the Perform CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
