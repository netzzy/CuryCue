...
This class inherits from the [ DAT class](<./DAT_Class.md> "DAT Class"). It references a specific [Evaluate DAT](<./Evaluate_DAT.md> "Evaluate DAT"). 

## Members`exprCell`→ **(Read Only)** : 

> The current expression [cell](<./Cell_Class.md> "Cell Class") being evaluated. From the [cell](<./Cell_Class.md> "Cell Class") you can get its row, column and value. e.g. me.exprCell.val or use the specific members listed below. Too see how a [Cell Class](<./Cell_Class.md> "Cell Class") such as me.exprCell is automatically converted to a string or number in an expression see [Casting to a Value](<./Cell_Class.htm#Casting_to_a_Value> "Cell Class").`exprCol`→`int`**(Read Only)** : 

> The current expression column being evaluated.`exprRow`→`int`**(Read Only)** : 

> The current expression row being evaluated.`exprTable`→`DAT`**(Read Only)** : 

> The input [DAT](<./DAT_Class.md> "DAT Class") containing the expressions.`inputCell`→`Cell`**(Read Only)** : 

> The current input [cell](<./Cell_Class.md> "Cell Class") being evaluated. From the [cell](<./Cell_Class.md> "Cell Class") you can get its row, column and value. e.g. me.inputCell.val or use the specific members listed below. To see how a [Cell Class](<./Cell_Class.md> "Cell Class") such as me.inputCell is automatically converted to a string or number in an expression see [Casting to a Value](<./Cell_Class.htm#Casting_to_a_Value> "Cell Class").`inputCol`→`int`**(Read Only)** : 

> The current input column being evaluated.`inputRow`→`int`**(Read Only)** : 

> The current input row being evaluated.`inputTable`→`DAT`**(Read Only)** : 

> The input [DAT](<./DAT_Class.md> "DAT Class") containing the source values.
