...
For advice on setting up the tablet and tuning the sensitivity of the pen and tablet, please read the [Wacom Intuos](<./Wacom_Intuos_Tablet.md> "Wacom Intuos Tablet") article. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[tabletCHOP_Class](<./TabletCHOP_Class.md> "TabletCHOP Class")
...
Active`active`\- ⊞ \- While **On** , the pen movement will be output from and the CHOP will cook every frame. When set to **Off** it will not cook and the values will not be updated. **While Playing** will capture pen events only when the [Timeline](<./Timeline.md> "Timeline") is playing forward. 
* Off`off`-
* On`on`-
* While Playing`play`-
...
Extra Information for the Tablet CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
