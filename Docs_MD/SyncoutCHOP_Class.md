...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP"). 

## Members`timecode`→`tdu.Timecode`**(Read Only)** : 

> Get a Timecode object for the timecode data representation of the last sent index. See [Timecode Class](<./Timecode_Class.md> "Timecode Class").

## Methods`send(message)`→`None`: 
