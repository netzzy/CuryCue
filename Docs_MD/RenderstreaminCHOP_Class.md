...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [RenderStream In CHOP](<./RenderStream_In_CHOP.md> "RenderStream In CHOP"). 

## Members`cameraTransform`→ **(Read Only)** : 

> Gets the camera transform matrix for the stream. This should be referenced in the 'Xform Matrix/CHOP/DAT' parameter of the Camera COMP.`projection`→ **(Read Only)** : 
