...
The Video Stream In TOP creates a client to receive video and audio across the network from RTSP, HLS, or SRT protocol sources; or from a WebRTC peer via a [WebRTC DAT](<./WebRTC_DAT.md> "WebRTC DAT"). 

The URL to connect to a RTSP server is in the form:`rtsp://<ipaddress>:<port>/<streamName>`for example`rtsp://192.168.0.1:554/tdvidstream`If the server requires a username/password, those can be specified using the form.`rtsp://username:password@192.168.0.1:554/tdvidstream`Access **[HLS/DASH](<https://en.wikipedia.org/wiki/HTTP_Live_Streaming>)** input streams via URLs that point to m3u8 files. 
...
See also [Audio Stream In CHOP](<./Audio_Stream_In_CHOP.md> "Audio Stream In CHOP"), [WebRTC DAT](<./WebRTC_DAT.md> "WebRTC DAT"). 

SRT sent in the Video Stream In TOP can include per-frame metadata making it easy to send and receive CHOP/DAT data in sync with video. Attach an [Info DAT](<./Info_DAT.md> "Info DAT") to the TOP. 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[videostreaminTOP_Class](<./VideostreaminTOP_Class.md> "VideostreaminTOP Class")
...
WebRTC DAT`webrtc`\- Set the [WebRTC DAT](<./WebRTC_DAT.md> "WebRTC DAT") (ie. peer) to get the video stream from. Setting this will automatically populate the WebRTC Connection parameter menu with available connections. 
...
Extra Information for the Video Stream In TOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). 
