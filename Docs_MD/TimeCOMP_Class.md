...
This class inherits from the [ COMP class](<./COMP_Class.md> "COMP Class"). It references a specific [Time COMP](<./Time_COMP.md> "Time COMP").   
  
## Members`frame`→`float`: 

> Get or set the current frame output by this component.`seconds`→`float`: 

> Get or set the current time output by this component (expressed in seconds).`rate`→`float`: 

> Get or set the frames per second, (or rate).`play`→`bool`: 

> Get or set whether the component is playing.`timecode`→`str`: 

> Get or set the current timecode generated by this component.

=== Range ===`start`→`float`: 

> Get or set start of main frame range.`end`→`float`: 

> Get or set end of main frame range.`rangeStart`→`float`: 

> Get or set start of sub frame range. Must be within main start, end range`rangeEnd`→`float`: 

> Get or set end of sub frame range. Must be within main start, end range.`loop`→`bool`: 

> Get or set whether the timeline loops.`independent`→`float`: 

> Get or set whether the timeline runs independently of other timelines.

=== Beats ===`tempo`→`float`: 

> Get or set beats per minute.`signature1`→`int`: 

> Get or set time signature, first value.`signature2`→`int`: 
...
> Time Component that defines the operator's time reference.`fileFolder`→`str`**(Read Only)** : 
