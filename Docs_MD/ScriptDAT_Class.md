...
This class inherits from the [ DAT class](<./DAT_Class.md> "DAT Class"). It references a specific [Script DAT](<./Script_DAT.md> "Script DAT"). 

## Members`textFormat`→`bool`: 
...
These methods can be used by including them in the`onCook()`callback of the Script DAT. Using them outside the callback may cause unexpected results.`appendCustomPage(name)`→`Page`: 
...
> Remove all custom parameters from the Script DAT. Returns number of destroyed pars.`sortCustomPages(*pages)`→`None`: 
