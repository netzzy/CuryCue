...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [Copy CHOP](<./Copy_CHOP.md> "Copy CHOP"). 

## Members`chanIndex`→`int`**(Read Only)** : 

>`me.chanIndex`can be used in any parameter to give a different value for each [channel](<./Channel_Class.md> "Channel Class") being generated, for example`[3, 4, 5][me.chanIndex]`.`sampleIndex`→`int`**(Read Only)** : 

> The index of the current sample being evaluated.`copyIndex`→`int`**(Read Only)** : 
