...
## Members`loaded`→`bool`**(Read Only)** : 

> The loaded state of the current webpage.

## Methods`interactMouse(u, v, leftClick=0, middleClick=0, rightClick=0, left=False, middle=False, right=False, wheel=0, pixels=False)`→`None`: 
...
>     
[/code]`interactTouch(u, v, pixels=False, radiusx=1, radiusy=1, start=-1, move=-1, end=-1)`→`None`: 
...
>     
[/code]`executeJavaScript(script)`→`None`: 
...
> 
>   * key - ASCII value or name of a character or key. Key name matches the names from the [Keyboard In DAT](<./Keyboard_In_DAT.md> "Keyboard In DAT").
