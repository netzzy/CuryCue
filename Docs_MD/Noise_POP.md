...
The Gradient is the rate-of-change of a scalar noise around each point in X Y and Z (aka the "first order derivative") and is a float3 named`NoiseGradient`Curl is an expression of how much the slope twists around each point. A 3D Curl is a float3 named`NoiseCurl`. A 2D Curl is float2 named`NoiseCurl2`If Combine Operation is Add (default) or Multiply, a float3 noise is always applied to`P`, even though the noise may be set to scalar. With Combine Attribute Scope, the noise can be applied to any scalar or vector attribute (one component to 4 components), and output to any new or existing attribute. 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[noisePOP_Class](<./NoisePOP_Class.md> "NoisePOP Class")
...
Extra Information for the Noise POP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
