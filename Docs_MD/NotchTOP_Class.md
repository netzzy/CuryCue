...
## Members`exposedProperties`→`dict`**(Read Only)** : 

> Return a dictionary of key/value pairs representing all the exposed properties imported from the Notch block. The key is the UUID and the value is the corresponding [ParGroup](<./ParGroup.md> "ParGroup") representation of the property.
