...
The Clip DAT contains information about motion clips that are manipulated by a [Clip CHOP](<./Clip_CHOP.md> "Clip CHOP") and [Clip Blender CHOP](<./Clip_Blender_CHOP.md> "Clip Blender CHOP"). The Clip DAT can hold any command or script text, which can be triggered based on the settings on the Execute parameter page (This is where the Clip DAT and the Text DAT are different). The Clip DAT's script is triggered by specified clips being played through a [Clip Blender CHOP](<./Clip_Blender_CHOP.md> "Clip Blender CHOP"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[clipDAT_Class](<./ClipDAT_Class.md> "ClipDAT Class")
...
Clip`clip`\- Points to the [Clip CHOP](<./Clip_CHOP.md> "Clip CHOP") which will trigger the script when run in a [Clip Blender CHOP](<./Clip_Blender_CHOP.md> "Clip Blender CHOP"). 
...
Extra Information for the Clip DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
