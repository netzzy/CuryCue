...
## Members`numLines`→`int`**(Read Only)** : 

> Get the number of lines of the outputted text, after operations such as word-wrap have been applied.`ascender`→`float`**(Read Only)** : 

> The ascender of the font, as described by the font's metrics.`descender`→`float`**(Read Only)** : 

> The descender of the font, as described by the font's metrics.`capHeight`→`float`**(Read Only)** : 

> The cap height of the font, as described by the font's metrics. This is usually the height of a capital H.`xHeight`→`float`**(Read Only)** : 

> The x height of the font, as described by the font's metrics. This is usually the height of a lower case x.`lineGap`→`float`**(Read Only)** : 

> The suggested gap between lines, as described by the font's metrics.`numGlyphs`→`int`**(Read Only)** : 
...
## Methods`fontSupportsChars(str)`→`bool`: 
