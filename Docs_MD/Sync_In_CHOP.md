...
The Sync In CHOP and [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP") are used to keep timelines in two or more TouchDesigner processes within a single frame of each other. One process will contain a [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP") while one or more other processes contain Sync In CHOPs. The processes with Sync In CHOPs should have their **Realtime flag checked off** , as their frame rates will be determined by the [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP"). Also note, all monitors (including all clients and server) should be set to the **same rate**. Note that unplugging or re-adding monitors may sometime change previously configured settings. 

The CHOPs synchronize by pausing their own timeline until all Sync In/Out CHOPs have cooked. The [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP") will be ahead of the Sync In CHOPs. If any CHOPs fail to communicate, the others will timeout, causing all processes to run slowly. 

Client machines may come online at any point, or be switched off as desired, as the [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP") will adjust accordingly, either timing out, temporarily or permanently banning individual clients as specified. 

In addition any extra CHOP channels sent through the [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP") is received by the Sync In CHOPs. 

**NOTE for Windows OS - If experiencing connection issues make sure Windows Firewall is disabled.**

An [Info DAT](<./Info_DAT.md> "Info DAT"), pointing to the [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP"), provides a detailed list of all clients: 
...
Here are two simple example files that use Sync In and Out CHOPs. You would run the SyncServer.toe only once, and run the SyncClient.toe for every machine you want to display on. Note that you may need to adjust the [Window COMP](<./Window_COMP.md> "Window COMP") located at /perform to output to your desired monitor. Also note that [Hardware Frame Lock](<./Hardware_Frame_Lock.md> "Hardware Frame Lock") is not enabled in these examples, so if you want to use that feature you'll need to turn that parameter on in the [Window COMP](<./Window_COMP.md> "Window COMP") for SyncClient.toe as well. 

[File:SyncServer.toe](</File:SyncServer.toe> "File:SyncServer.toe")  
[File:SyncClient.toe](</File:SyncClient.toe> "File:SyncClient.toe")

## Intermittent Connections

Whenever a client replies, it is immediately assumed reliable, and waited upon, by the [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP") each frame thereafter. When a client times-out a number of times in a row, however, it is continuously ignored by the [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP"), until it replies on time again. This also applies to clients that have stopped communicating altogether. The number of consecutive timeouts is controlled by the`Client Timeouts (consecutive)`parameter and is reflected by the`timeout_consecutive`[Info DAT](<./Info_DAT.md> "Info DAT") column. 
...
Whenever the Sync Out CHOP first starts any and all clients are accepted, and nothing is banned. Pressing`Clear Stats`in the [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP") also returns to this state, clearing all banned lists, and totals, even in the remote client [Info CHOPs](<./Info_CHOP.md> "Info CHOP").   
  
See also [Syncing Multiple Computers](<./Syncing_Multiple_Computers.md> "Syncing Multiple Computers") and [Hardware Frame Lock](<./Hardware_Frame_Lock.md> "Hardware Frame Lock"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[syncinCHOP_Class](<./SyncinCHOP_Class.md> "SyncinCHOP Class")

## 

Parameters - Sync In Page

Active`active`\- Whether or not the CHOP is currently attempting to synchronize itself to a [Sync Out CHOP](<./Sync_Out_CHOP.md> "Sync Out CHOP"). 
...
Extra Information for the Sync In CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). 
