...
**NOTE:** For TouchDesigner-to-TouchDesigner communication of channel data, see the [Touch In CHOP](<./Touch_In_CHOP.md> "Touch In CHOP") and [Touch Out CHOP](<./Touch_Out_CHOP.md> "Touch Out CHOP") which offer optimized and [Time Sliced](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)") data transfer. 

**TIP:** The [TCP/IP DAT](<./TCP/IP_DAT.md> "TCP/IP DAT") or [UDP Out DAT](<./UDP_Out_DAT.md> "UDP Out DAT") is the preferred way to send script commands and other data packets over the network. 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[pipeoutCHOP_Class](<./PipeoutCHOP_Class.md> "PipeoutCHOP Class")
...
Send Current Sample Only`sendsingle`\- This parameter is only enabled if the Pipe Out CHOP is not [time sliced](<./Time_Slicing.md> "Time Slicing") (See Common Page of parameters). If On, it only sends the current frame's sample. If Off, it sends all data between this frame and the last frame TouchDesigner cooked at. The difference arises when TouchDesigner is not cooking 60 frames per second and is skipping frames. If playback skips by 3 animation frames between screen draws, the On case sends only one frame of data, and the Off case sends 3 frames of data (a "[Time Slice](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)")" in TouchDesigner-speak). 
...
Callbacks DAT`callbacks`\- Path to a DAT containing callback methods for each event sent. See [pipeoutCHOP_Class](<./PipeoutCHOP_Class.md> "PipeoutCHOP Class") for usage. 
