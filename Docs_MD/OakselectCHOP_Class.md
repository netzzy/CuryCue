...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [OAK Select CHOP](<./OAK_Select_CHOP.md> "OAK Select CHOP"). 

## Members`oakBuffer`→ **(Read Only)** : 

> Gets the most recent depthai.Buffer received by the OAK Select CHOP. See <https://docs.luxonis.com/projects/api/en/latest/components/messages/buffer/>`oakNnData`→ **(Read Only)** : 

> Gets the most recent depthai.NNData received by the OAK Select CHOP. See <https://docs.luxonis.com/projects/api/en/latest/components/messages/nn_data/>`oakSysInfo`→ **(Read Only)** : 

> Gets the most recent depthai.SystemInformation received by the OAK Select CHOP. See <https://docs.luxonis.com/projects/api/en/latest/components/messages/system_information/>`timeSliceDefault`→ **(Read Only)** : 

> Get the default Time Slice for the OAK Select CHOP.`timecode`→`tdu.Timecode`**(Read Only)** : 

> Get the master timecode from the referenced OAK Device CHOP.

## Methods`appendChan(name)`→`Channel`: 
...
>     
[/code]`appendCustomPage(name)`→`Page`: 
...
> Remove all channels from the CHOP. The channel length, sample rate etc. remain unchanged.`copy(chop)`→`None`: 
...
> Remove all custom parameters from the OAK Select CHOP. Returns number of destroyed pars.`sortCustomPages(*pages)`→`None`: 
