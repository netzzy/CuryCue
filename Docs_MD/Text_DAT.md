...
See also the Execute DATs which are specialized to run their text as a script: [CHOP Execute DAT](<./CHOP_Execute_DAT.md> "CHOP Execute DAT") runs its script when a CHOP channel changes, [DAT Execute DAT](<./DAT_Execute_DAT.md> "DAT Execute DAT") when a DAT changes, [Execute DAT](<./Execute_DAT.md> "Execute DAT") when you start or end your TouchDesigner process or want to run a script every frame, [Panel Execute DAT](<./Panel_Execute_DAT.md> "Panel Execute DAT") when a control panel changes, [Parameter Execute DAT](<./Parameter_Execute_DAT.md> "Parameter Execute DAT") when a parameter of a node changes, and [OP Execute DAT](<./OP_Execute_DAT.md> "OP Execute DAT") when anything else about an operator changes, including creation and deletion of nodes in a component's network. 

Use the [Web DAT](<./Web_DAT.md> "Web DAT") to fetch via a URL query. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[textDAT_Class](<./TextDAT_Class.md> "TextDAT Class")
...
Extra Information for the Text DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
