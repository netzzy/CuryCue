...
The Keyframe CHOP uses channel and keys data in an [Animation COMP](<./Animation_COMP.md> "Animation COMP") and creates channels of samples at a selectable sample rate (frames per second). 

See also: [Animation COMP](<./Animation_COMP.md> "Animation COMP") and [Animation Editor](<./Animation_Editor.md> "Animation Editor")
...
To create/edit/delete keyframed channels, create an [Animation COMP](<./Animation_COMP.md> "Animation COMP") and open the [Animation Editor](<./Animation_Editor.md> "Animation Editor") by right-clicking on the component and selecting Edit Keyframes... from the pop-up menu. Inside the Animation component a Keyframe CHOP will be outputting the channels created in the editor. 

Adding an input to the Keyframe CHOP now acts as a lookup index. In addition it outputs a proper [Time Slice](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)") if its input is a Time Slice. 

The lookup cycles through all the input channels, though only 1 is necessary. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[keyframeCHOP_Class](<./KeyframeCHOP_Class.md> "KeyframeCHOP Class")
