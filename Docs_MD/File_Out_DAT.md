...
The File Out DAT allows you to write out DAT contents to a`.dat`file or a`.txt`file. A`.dat`file is one of the [File Types](<./File_Types.md> "File Types") of TouchDesigner that is used to hold the arrays of the [Table DAT](<./Table_DAT.md> "Table DAT"). 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[fileoutDAT_Class](<./FileoutDAT_Class.md> "FileoutDAT Class")
...
N`n`\- Using`me.par.n`(or $N in [Tscript](<./Tscript.md> "Tscript")) in the filename (in the File parameter) in conjuction with the N parameter here gives a method of incrementing file names. The N parameter must manually be incremented each time. Exporting a [Count CHOP](<./Count_CHOP.md> "Count CHOP") which increments each time you want to save out a new file is an easy way to do this. 
...
Extra Information for the File Out DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
