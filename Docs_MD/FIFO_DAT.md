...
The FIFO DAT maintains a user-set maximum number of rows in a table. You add rows using the`appendRow()`method found in [DAT Class](<./DAT_Class.md> "DAT Class"). When its capacity is reached, the first row is removed. After the maximum number of rows is reached, the oldest row is discarded when a new row is added. 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[fifoDAT_Class](<./FifoDAT_Class.md> "FifoDAT Class")
...
Clear Output`clear`\- Deletes all lines except the heading. To clear with a python script`op('opname').par.clear.pulse()`Keep First Row`firstrow`\- Keeps first row in table. 
...
Extra Information for the FIFO DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
