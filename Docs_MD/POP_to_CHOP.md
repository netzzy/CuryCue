...
The format of the channel names is limited since you cannot have`()`or`[]`in channel names. The Name Format menu lets you choose between Basic: a simple CHOP-like format with lowercase letters with numbers following, or Precise format, where`()`is represented by _ and`[]`is represented by _ in a form that can be interpreted exactly by [CHOP to POP](<./CHOP_to_POP.md> "CHOP to POP") when returning channels back to POPs. 
...
See also [CHOP to POP](<./CHOP_to_POP.md> "CHOP to POP"), [POP to TOP](<./POP_to_TOP.md> "POP to TOP"), [POP to DAT](<./POP_to_DAT.md> "POP to DAT")

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[poptoCHOP_Class](</PoptoCHOP_Class> "PoptoCHOP Class")
...
Extra Information for the POP to CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
