...
The NDI In TOP will obtain its image data over IP from other DI® (Network Data Interface) enabled applications. The [NDI®](<./NDI.md> "NDI") protocol is created by [Newtek](<http://www.newtek.com/ndi/applications/>). 

Attach an [Audio NDI CHOP](<./Audio_NDI_CHOP.md> "Audio NDI CHOP") to the NDI In DAT to extract audio from the NDI stream. 

Metadata can also be sent to the NDI In TOP from an [NDI Out TOP](<./NDI_Out_TOP.md> "NDI Out TOP") or another system that has attached metadata to the NDI stream. Attach an [Info DAT](<./Info_DAT.md> "Info DAT") to the NDI In DAT to get the metadata as text. It is in an XML format. To better see and access the XML, attach an [XML DAT](<./XML_DAT.md> "XML DAT") to the Info DAT. 
...
To see details of the available NDI streams, use the [NDI DAT](<./NDI_DAT.md> "NDI DAT"). 

See also [NDI](<./NDI.md> "NDI"), [NDI Out TOP](<./NDI_Out_TOP.md> "NDI Out TOP") and [NDI DAT](<./NDI_DAT.md> "NDI DAT"). 

**NOTE for Windows OS - If experiencing connection issues, check Windows firewall settings.**

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[ndiinTOP_Class](<./NdiinTOP_Class.md> "NdiinTOP Class")
...
Extra Information for the NDI In TOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). 
