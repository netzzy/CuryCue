...
These methods can be used by including them in the onCook callback of the Script SOP. Using them outside the callback may cause unexpected results.`clear()`→`None`: 
...
> Append a [poly](<./Poly_Class.md> "Poly Class") to this SOP. Returns the appended polygon. The returned polygon, is a list of vertices, and if`addPoints=True`, then`polygon[0].point.x`can be set to the x value of the first point of the polygon, for example. 
> 
...
>   * addPoints - (Keyword, Optional) If True, a new [point](<./Point_Class.md> "Point Class") will be attached to each [vertex](<./Vertex_Class.md> "Vertex Class"), otherwise the [vertex](<./Vertex_Class.md> "Vertex Class") point references will need to be manually set afterwards. Manually add them when creating [polygons](<./Poly_Class.md> "Poly Class") with shared [vertices](<./Vertex_Class.md> "Vertex Class").
...
> Creates a point [group](<./Group_Class.md> "Group Class") with input string name. Raises an exception if the group already exists.`createPrimGroup(str)`→`None`: 

> Creates a primitive [group](<./Group_Class.md> "Group Class") with input string name. Raises an exception if the group already exists.`appendCustomPage(name)`→`Page`: 

> Add a new [page](<./Page_Class.md> "Page Class") of custom parameters. See [Page Class](<./Page_Class.md> "Page Class") for more details. 
[code]
...
> Remove all custom parameters from the Script SOP. Returns number of destroyed pars.`sortCustomPages(*pages)`→`None`: 
...
## Methods`bounds(display=False, render=False)`→`Bounds`: 
...
> 
>   * display - (Keyword, Optional) If set to True, only [SOP operators](<./SOP_Class.md> "SOP Class") whose[ display flag](<./Display_Flag.md> "Display Flag") are turned on are included in the calculation. When False, the states of the display flag is ignored.
>   * render - (Keyword, Optional) If set to True, only [SOP operators](<./SOP_Class.md> "SOP Class") whose[ render flag](<./Render_Flag.md> "Render Flag") are turned on are included in the calculation. When False, the state of the render flag is ignored.
...
TouchDesigner Build: Latest\nwikieditorwikieditorwikieditorwikieditorwikieditor2025.300002021.100002020.200002018.28070before 2018.28070
