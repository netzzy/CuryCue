...
This class inherits from the [ DAT class](<./DAT_Class.md> "DAT Class"). It references a specific [WebRTC DAT](<./WebRTC_DAT.md> "WebRTC DAT"). 

## Members`peerConnections`→ **(Read Only)** : 

> Returns a list of all peer connection IDs.

## Methods`addIceCandidate(connectionId, candidate, lineIndex, sdpMid)`→`None`: 
...
> * connectionId - (Keyword, Optional) The UUID of the connection.`getSignalingState(connectionId)`→`str`: 

> * connectionId - (Keyword, Optional) The UUID of the connection.`getIceGatheringState(connectionId)`→`str`: 

> * connectionId - (Keyword, Optional) The UUID of the connection.`getIceConnectionState(connectionId)`→`str`: 

> * connectionId - (Keyword, Optional) The UUID of the connection.`getStats(connectionId=None)`→`str`: 
...
> Opens the local end of a peer connection and returns the UUID.`removeTrack(connectionId, trackId, type)`→`bool`: 
