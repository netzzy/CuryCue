...
The DAT to CHOP will create a set of CHOP channels with values derived from a [DAT](<./DAT.md> "DAT"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[dattoCHOP_Class](<./DattoCHOP_Class.md> "DattoCHOP Class")

## 

Parameters - DAT to Page

DAT`dat`\- The DAT to be used to retrieve values. This DAT should be in _table_ format. For table format, use either a [Table DAT](<./Table_DAT.md> "Table DAT") or a [Convert DAT](<./Convert_DAT.md> "Convert DAT") set to **To Table**. 
...
Extra Information for the DAT to CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
