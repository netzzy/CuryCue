...
These methods can be used by including them in the onCook callback of the Script TOP. Using them outside the callback may cause unexpected results.`copyNumpyArray(numpyArray, is3D=False, isCube=False)`→`None`: 
...
> Copies the contents of the ptr, with given size and shape ([CUDAMemoryShape](<./CUDAMemoryShape_Class.md> "CUDAMemoryShape Class")) into the TOPs texture. If you obtained your data from`cudaMemory()`, these pieces of data are available in the [CUDAMemory](<./CUDAMemory_Class.md> "CUDAMemory Class") that`cudaMemory()`returns. 
> 
>   * address - The source CUDA memory address.
>   * size - The size of the CUDA memory.
>   * shape - A [CUDAMemoryShape](<./CUDAMemoryShape_Class.md> "CUDAMemoryShape Class") object describing the shape of the texture to create. See the help for that class for notes about the channel order requirements for different data types.
...
> Add a new page of custom parameters. See [Page Class](<./Page_Class.md> "Page Class") for more details. 
[code]
...
> Remove all custom parameters from the Script TOP. Returns number of destroyed pars.`sortCustomPages(*pages)`→`None`: 
...
TouchDesigner Build: Latest\nwikieditorwikieditorwikieditorwikieditor2025.300002021.10000before 2021.10000
