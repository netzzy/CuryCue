...
The Audio Stream In CHOP can stream audio into TouchDesigner from any [rtsp](<http://en.wikipedia.org/wiki/Real_Time_Streaming_Protocol>) server, or from a [WebRTC](<./WebRTC.md> "WebRTC") peer. See [Video Stream In TOP](<./Video_Stream_In_TOP.md> "Video Stream In TOP"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[audiostreaminCHOP_Class](<./AudiostreaminCHOP_Class.md> "AudiostreaminCHOP Class")
...
Video Stream In TOP`videostreamintop`\- Point to a [Video Stream In TOP](<./Video_Stream_In_TOP.md> "Video Stream In TOP") whose stream has audio embedded to extract the audio channels. 
...
Audio Sync Offset`syncoffset`\- Offsets the audio playback of the movie. This can be used to get better sync between the audio and images in the stream when there is audio latency in the system (For example, audio latency from the [Audio Device Out CHOP](<./Audio_Device_Out_CHOP.md> "Audio Device Out CHOP") queue). A negative value plays audio that amount of time sooner, to counteract delay introduced by buffering such as in [Audio Device Out CHOP](<./Audio_Device_Out_CHOP.md> "Audio Device Out CHOP"). 
...
WebRTC DAT`webrtc`\- Set the [WebRTC DAT](<./WebRTC_DAT.md> "WebRTC DAT") (ie. peer) to get the audio stream from. Setting this will automatically populate the WebRTC Connection parameter menu with available connections. 

WebRTC Connection`webrtcconnection`\- Select the [WebRTC](<./WebRTC.md> "WebRTC") peer-to-peer connection. Selecting this will automatically population the WebRTC Track parameter menu with available audio input tracks. 
