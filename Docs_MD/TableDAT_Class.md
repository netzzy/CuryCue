...
This class inherits from the [ DAT class](<./DAT_Class.md> "DAT Class"). It references a specific [Table DAT](<./Table_DAT.md> "Table DAT").   
  
## Members`subRow`→`int`**(Read Only)** : 

> Current row index for Table expressions.`subCol`→`int`**(Read Only)** : 

> Current col index for Table expressions.`fillName`→`str`**(Read Only)** : 
