...
The Handle CHOP is the "engine" which drives Inverse Kinematic solutions using the [Handle COMP](<./Handle_COMP.md> "Handle COMP"). The role of the Handle CHOP is to generate rotation values for the bones which will bring their attached handles as close to their respective targets as possible. 

The use of this method is best described with an example: 
1. Increase the frame range to 10000. Create a [Null COMP](<./Null_COMP.md> "Null COMP") called 'target' and animate a few random positions for it to travel to.
  2. Create a three bone IK chain with No Kinematics. Append a [Handle COMP](<./Handle_COMP.md> "Handle COMP") to the last bone.
...
**Tip:** In the [geometry viewer](<./Geometry_Viewer.md> "Geometry Viewer"), you can use the **Select** state to select the bones you created. Then in the Handle CHOP click the **Grab Source Form Selection** button. The names are entered automatically for you. You don't have to worry about being too picky when selecting objects this way, since non-bone objects are ignored. 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[handleCHOP_Class](<./HandleCHOP_Class.md> "HandleCHOP Class")
...
Extra Information for the Handle CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
