...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[directxinTOP_Class](<./DirectxinTOP_Class.md> "DirectxinTOP Class")

## 

Parameters - DirectX In Page

Handle`handle`\- Specify the handle pointing to the DirectX texture. You can get the handle of any [DirectX Out TOP](<./DirectX_Out_TOP.md> "DirectX Out TOP") texture by attaching an [Info DAT](<./Info_DAT.md> "Info DAT") to it. The`handle`is one of the columns. Other applications that support sharing texture will have their own way of getting a texture handle, please consult the application's documentation. 
...
Extra Information for the DirectX In TOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
