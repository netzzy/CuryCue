...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[glslmultiTOP_Class](<./GlslmultiTOP_Class.md> "GlslmultiTOP Class")
...
Compute Shader`computedat`\- Points to the [DAT](<./DAT.md> "DAT") holding the [Compute Shader](<./Compute_Shader.md> "Compute Shader"). Drag & Drop a DAT here, or manually enter the path to the DAT. 
...
Output Type`type`\- ⊞ \- Specify what type of texture to create. When creating a 3D texture the TOP will render once for every slice of the output. Refer to [ 3D Textures and 2D Texture Arrays](<./Write_a_GLSL_TOP.htm#3D_Textures_and_2D_Texture_Arrays> "Write a GLSL TOP") for more info. 
* 2D Texture`texture2d`\- Creates a 2D texture.
* 2D Texture Array`texture2darray`\- Creates a [2D Texture Array](<./2D_Texture_Array.md> "2D Texture Array"). Slices of the array can be access using a non-normalized integer index for the w coordinate.
* 3D Texture`texture3d`\- Creates a [3D Texture](<./3D_Texture.md> "3D Texture"). Slices of the array can be accessed using the w coordinate in the range 0-1. Value of the texture in between slices are interpolated.
...
Input Extend Mode W`inputextendw`\- ⊞ \- Controls what is returned from your texture sampling functions when the W texture coordinate (called W in the shader) are outside [0-1] range. Only useful for [3D Texture](<./3D_Texture.md> "3D Texture"). 
* Hold`hold`-
* Zero`zero`-
* Repeat`repeat`-
* Mirror`mirror`-
...
Matrix`matrix0value`\- The value to assign the matrix. For valid ways to specify this, see the [Matrix Parameters](<./Matrix_Parameters.md> "Matrix Parameters") article. 
...
[Specialization Constants](<./Write_a_GLSL_TOP.htm#Specialization_Constants> "Write a GLSL TOP") can optionally have their values assigned here. 
...
Extra Information for the GLSL Multi TOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
