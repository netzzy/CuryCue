...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [Audio Oscillator CHOP](<./Audio_Oscillator_CHOP.md> "Audio Oscillator CHOP"). 

## Members`chanIndex`→`int`**(Read Only)** : 

>`me.chanIndex`can be used in any parameter to give a different value for each [channel](<./Channel_Class.md> "Channel Class") being generated, for example`[3, 4, 5][me.chanIndex]`.
