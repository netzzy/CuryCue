...
## Members`curText`→`str`**(Read Only)** : 

> Current text contents, when used with a [Field COMP](<./Field_COMP.md> "Field COMP").`cursorEnd`→`int`: 

> Get or set cursor end position, when used with a [Field COMP](<./Field_COMP.md> "Field COMP").`cursorStart`→`int`: 

> Get or set cursor start position, when used with a [Field COMP](<./Field_COMP.md> "Field COMP").`selectedText`→`str`**(Read Only)** : 

> Selected contents, when used with a [Field COMP](<./Field_COMP.md> "Field COMP").`textHeight`→`int`**(Read Only)** : 

> Calculated height of text, in pixels. This value does **not** changes based on the particular of characters in the string. It only depends on the number of lines, line spacing, positioning and font metrics of the font.`textWidth`→`int`**(Read Only)** : 

> Calculated width of text, in pixels. This value **does** change depending on the particular characters in the string. Different characters have difference advance widths, and this value is the sum of all the advance widths of the characters.`numLines`→`int`**(Read Only)** : 
