...
When the Time Slice flag on the Common parameter page is on, the Merge CHOP will merge only the current [Time Slice](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)") of a channel, not the entire length of all the channels. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[mergeCHOP_Class](<./MergeCHOP_Class.md> "MergeCHOP Class")
...
* Automatic`auto`\- It will trim to the current [Time Slice](</index.php?title=Time_Slice&action=edit&redlink=1> "Time Slice \(page does not exist\)") if there are any Time Slices, else it will do an Extend Min/Max.
* Extend to Min/Max`none`\- Find the earliest start and latest end, and extend all inputs to that range using the extend conditions. (see [Extend Conditions](<./CHOP_Channel_Page.htm#Extend_Conditions> "CHOP Channel Page")).
...
Extra Information for the Merge CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
