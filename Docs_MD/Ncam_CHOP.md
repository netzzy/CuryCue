...
The Ncam CHOP receives camera tracking data from an external [Ncam Reality system](<https://www.ncam-tech.com/>) for use in virtual production. The data is received over a network using the TCP protocol and includes information on the camera's position, orientation and optical properties. This data can be used in a [Camera COMP](<./Camera_COMP.md> "Camera COMP") to render content from a virtual environment that is synced to the movement of a physical camera. An [Ncam TOP](<./Ncam_TOP.md> "Ncam TOP") can also be used to receive image data from the camera and to composite it with the rendered content. 

For additional tracking solutions, see the [Stype CHOP](<./Stype_CHOP.md> "Stype CHOP") and [FreeD CHOP](<./FreeD_CHOP.md> "FreeD CHOP"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[ncamCHOP_Class](<./NcamCHOP_Class.md> "NcamCHOP Class")
