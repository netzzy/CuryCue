...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [Audio File In CHOP](<./Audio_File_In_CHOP.md> "Audio File In CHOP"). 

## Members`timecode`→`tdu.Timecode`**(Read Only)** : 

> Get a Timecode object for timecode data representation of the current index of the audio file. See [Timecode Class](<./Timecode_Class.md> "Timecode Class").`metadata`→`dict`**(Read Only)** : 
