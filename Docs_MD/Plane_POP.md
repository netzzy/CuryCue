...
Dimension: When the grid is passed to other POPs it cannot be known for certain how many rows and columns were specified. However the Plane POP (and other generator POPs) output its [Dimensions](<./Dimension.md> "Dimension") as metadata to all POPs connected to it, indicating its organizations of points in the points list. A plane grid has dimensions`_numCols numRows_`, which can be used by built-in attributes like`_Dim[]`or by GLSL code. 
...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[planePOP_Class](<./PlanePOP_Class.md> "PlanePOP Class")
...
Extra Information for the Plane POP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
