...
## Members`calibrationParameters`→`dict`**(Read Only)** : 

> Dictionary containing the intrinsics and extrinsic stereo parameters for rectified/undistorted images. This includes focal point, optical center, distortion, field of view, and stereo transformation matrix.`calibrationParametersRaw`→`dict`**(Read Only)** : 
