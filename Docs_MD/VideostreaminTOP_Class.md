...
## Members`connectionsFailed`→`int`**(Read Only)** : 

> The number of times this operator has failed to make a connection to any URL.`connectionsLost`→`int`**(Read Only)** : 

> The number of times this operator has lost a connection it has previous successfully established.`frameTime`→`float`**(Read Only)** : 

> The timestamp of the currently shown frame, in seconds.`isConnected`→`bool`**(Read Only)** : 

> True if connected to target URL.`isConnecting`→`bool`**(Read Only)** : 

> True if attempting to connect to target URL.`isOddField`→`bool`**(Read Only)** : 

> When de-interlacing, this tells if the odd field is currently being shown.`videoHeight`→`int`**(Read Only)** : 

> Height of the movie, in pixels.`videoWidth`→`int`**(Read Only)** : 

> Width of the movie, in pixels.

## Methods`unload(cacheMemory=False)`→`None`: 
