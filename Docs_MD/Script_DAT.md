...
The Script DAT runs a script each time the DAT cooks and can build/modify the output table based in the optional input tables. The Script DAT is created with a [docked](<./Docking.md> "Docking") (attached) DAT that contains three Python methods:`cook`,`onPulse`, and`setupParameters`. The`cook`method is run each time the Script DAT cooks. The`setupParameters`method is run whenever the Setup Parameter button on the Script page is pressed. The`onPulse`method is run whenever a custom pulse parameter is pushed. 

Refer to Help -> Python Examples, and Help -> [Operator Snippets](<./OP_Snippets.md> "OP Snippets"). 
...
See also: [Script CHOP](<./Script_CHOP.md> "Script CHOP"), [Script SOP](<./Script_SOP.md> "Script SOP"), [Script TOP](<./Script_TOP.md> "Script TOP")

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[scriptDAT_Class](<./ScriptDAT_Class.md> "ScriptDAT Class")

## 

Parameters - Script Page

Callbacks DAT`callbacks`\- Specifies the DAT which holds the callbacks. See [scriptDAT_Class](<./ScriptDAT_Class.md> "ScriptDAT Class") for usage. 
...
Extra Information for the Script DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
