...
The SocketIO DAT connects to a [Socket.IO](<./Socket.md> "Socket.IO") server at the specified URL. 

The SocketIO DAT has 4 inputs in the following order: 
1. **Additional headers** in table format with two columns, structured as name/value pairs. Equivalent to [extraHeaders](<http://socket.io/docs/v3/client-initialization/#extraheaders>) option.
  ...
  3. **Query parameters** in table format with two columns, structured as name/value pairs. Equivalent to [query](<http://socket.io/docs/v3/client-initialization/#query>) option.
...
Extra Information for the SocketIO DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
