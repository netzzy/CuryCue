...
This class inherits from the [ POP class](<./POP_Class.md> "POP Class"). It references a specific [File Out POP](<./File_Out_POP.md> "File Out POP"). 

## Members`writeCount`→ **(Read Only)** : 

> The number of files that have been written. This can be used to create an incrementing file name.`curSeqIndex`→ **(Read Only)** : 

> The current index of the last written image on disk.`fileSuffix`→`str`**(Read Only)** : 
