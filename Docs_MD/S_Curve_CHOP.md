...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[scurveCHOP_Class](</ScurveCHOP_Class> "ScurveCHOP Class")
...
Sample Rate`rate`\- The sample rate of the channels, in samples per second. Default:`me.time.rate`Extend Left`left`\- ⊞ \- The left extend condition (before range). 
...
Extra Information for the S Curve CHOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
