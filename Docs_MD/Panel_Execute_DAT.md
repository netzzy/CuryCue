...
Panel Execute DATs are created with [default python method](<./PanelexecuteDAT_Class.md> "PanelexecuteDAT Class") placeholders. For each monitored condition in the parameters, there is a matching python method in the DAT. When a condition is turned on in the parameters, each time that condition is satisfied the corresponding python method will be executed. 

In the template Python script,`panelValue`is passed to each method, and you can query each call by printing`panelValue.name`and`panelValue.val`. See [PanelValue Class](<./PanelValue_Class.md> "PanelValue Class"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[panelexecuteDAT_Class](<./PanelexecuteDAT_Class.md> "PanelexecuteDAT Class")
...
Extra Information for the Panel Execute DAT can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
