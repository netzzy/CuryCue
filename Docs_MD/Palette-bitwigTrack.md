...
[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[Palette:bitwigTrack1 Ext](</index.php?title=Palette:bitwigTrack1_Ext&action=edit&redlink=1> "Palette:bitwigTrack1 Ext \(page does not exist\)")
...
Read Audio Envelope`Readaudioenvelope`\- Activates the listener for audio envelope, which is passed to the audioEnvelope channel in the output CHOP. The channel value represents the sum of the left and right channels. 

Crossfade Mode`Crossfademode`\- ⊞ \- The crossfade mode for the selected track. 
* A`A`-
* AB`AB`-
* B`B`-
...
Sync Comp Color to Track Color`Synccompcolortotrackcolor`\- Matches the bitwigTrack Component color to the color of the Bitwig Track Object it selects. When enabled, the Component color attribute will automatically match any changes made to the Bitwig Track color. 
...
Restore Automation Control`Restoreautomationcontrol`\- Will restore automation control for all sends on the current track.`Send0`\- The send amount for the 1st FX send on the current track.`Send1`\- The send amount for the 2nd FX send on the current track.`Send2`\- The send amount for the 3rd FX send on the current track.`Send3`\- The send amount for the 4th FX send on the current track.`Send4`\- The send amount for the 5th FX send on the current track.`Send5`\- The send amount for the 6th FX send on the current track.`Send6`\- The send amount for the 7th FX send on the current track.`Send7`\- The send amount for the 8th FX send on the current track. 
