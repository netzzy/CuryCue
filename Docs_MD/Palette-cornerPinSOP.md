# Palette:cornerPinSOP

## 

Summary

The Corner Pin SOP component allows for perspective or bilinear deformation of a SOP. It can be best compared in functionality to the [Corner Pin TOP](<./Corner_Pin_TOP.md> "Corner Pin TOP"). 

See also [Projection Mapping](<./Projection_Mapping.md> "Projection Mapping"). 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[Palette:cornerPinSOP Ext](<./Palette-cornerPinSOP_Ext.md> "Palette:cornerPinSOP Ext")

## 

Parameters - Pin Page

Help`Help`\- Opens up this page. 

Version`Version`\- The current version of this component. 

Mapping`Mapping`\- ⊞ \- Specify the mapping type of the operation. 
* Perspective`perspective`\- A perspective mapping uses a homography and a perspective transform on the geometry.
* Bilinear`bilinear`\- A bilinear mapping results in a linear redistribution of points inside the new area.


Bottom Left`Bottomleft`\- ⊞ \- Control the bottom left corner's x and y normalized offset. 
* Bottom Left`Bottomleftx`-
* Bottom Left`Bottomlefty`-


Bottom Right`Bottomright`\- ⊞ \- Control the bottom right corner's x and y normalized offset. 
* Bottom Right`Bottomrightx`-
* Bottom Right`Bottomrighty`-


Top Left`Topleft`\- ⊞ \- Control the top left corner's x and y normalized offset. 
* Top Left`Topleftx`-
* Top Left`Toplefty`-


Top Right`Topright`\- ⊞ \- Control the top right corner's x and y normalized offset. 
* Top Right`Toprightx`-
* Top Right`Toprighty`-
...
Operator Inputs
* Input 0: in1 \- The input SOP.

## 

Operator Outputs
* Output 0 \- The resulting SOP.
