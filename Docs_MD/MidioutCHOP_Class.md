...
This class inherits from the [ CHOP class](<./CHOP_Class.md> "CHOP Class"). It references a specific [MIDI Out CHOP](<./MIDI_Out_CHOP.md> "MIDI Out CHOP"). 
...
**Note:** the following event generation methods do not require any CHOP inputs connected to the MIDI Out CHOP.`send(*messages)`→`None`: 
...
>     n.sendBalance(1, 103)
>     
[/code]`sendNoteOn(channel, index, value)`→`None`: 
...
>     n.sendNoteOn(1, 63)
>     
[/code]`sendPolyKeyPressure(channel, index, value)`→`None`: 
...
>     
[/code]`sendPitchBend(channel, value)`→`None`: 
...
>     
[/code]`sendEffectsDepth(channel, index,value)`→`None`: 
...
>     
[/code]`sendMonoOn(channel,value)`→`None`: 
...
>     n.panic()
>     
[/code]`sendDamperPedal(channel,value)`→`None`: 
...
>     
[/code]`sendChannelPressure(channel, value)`→`None`: 
...
>     
[/code]`sendControl(channel, index, value)`→`None`: 
...
>     
[/code]`sendDataDecrement(channel,value)`→`None`: 
...
>     
[/code]`sendAllNotesOff(channel,value)`→`None`: 
...
>     
[/code]`sendSoundController(channel, index,value)`→`None`: 
...
>     
[/code]`sendBankSelect(channel,value)`→`None`: 
...
>     
[/code]`sendMainVolume(channel, value)`→`None`: 
...
>     
[/code]`sendSoftPedal(channel,value)`→`None`: 
...
>     
[/code]`sendLocalControl(channel,value)`→`None`: 
...
>     
[/code]`sendPolyOn(channel,value)`→`None`: 
...
>     n.sendPolyOn(1,16)
>     
[/code]`sendPan(channel, value)`→`None`: 
...
>     n.sendPan(1, 45)
>     
[/code]`sendResetAllControllers(channel,value)`→`None`: 
...
>     
[/code]`sendBreathController(channel, value)`→`None`: 
...
>     
[/code]`sendPortamentoTime(channel, value)`→`None`: 
...
>     
[/code]`sendDataIncrement(channel,value)`→`None`: 
...
>     
[/code]`sendFootController(channel, value)`→`None`: 
...
>     
[/code]`sendHold2(channel,value)`→`None`: 
...
>     n.sendHold2(1,55)
>     
[/code]`sendSostenuto(channel,value)`→`None`: 
...
>     
[/code]`sendOmniOff(channel,value)`→`None`: 
...
>     n.sendOmniOff(1,91)
>     
[/code]`sendGeneralPurposeController(channel, index,value)`→`None`: 
...
>     
[/code]`sendNoteOff(channel, index, value)`→`None`: 
...
>     n.sendNoteOff(1, 63)
>     
[/code]`sendExclusive(*messages)`→`None`: 
...
>     
[/code]`sendPortamento(channel,value)`→`None`: 
...
>     
[/code]`sendEffectControl(channel, index,value)`→`None`: 
...
>     
[/code]`sendLegatoFootswitch(channel,value)`→`None`: 
...
>     
[/code]`sendModulationWheel(channel,value)`→`None`: 
...
>     
[/code]`sendProgram(channel, value)`→`None`: 
...
>     n.sendProgram(1, 10)
>     
[/code]`sendOmniOn(channel,value)`→`None`: 
...
>     n.sendOmniOn(1,41)
>     
[/code]`sendPortamentoControl(channel,value)`→`None`: 
...
>     
[/code]`sendTimecode(timecode)`→`None`: 

> Sends the timecode to the device as a full MTC message. 
> 
>   * timecode - The timecode value to send to the device. Must be a tdu.Timecode object. Timecode rate must be 24, 25, 29.97 (drop-frame), or 30. See [Timecode Class](<./Timecode_Class.md> "Timecode Class").
> 
