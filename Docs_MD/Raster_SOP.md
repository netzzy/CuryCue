...
The Raster SOP converts TOP image data to geometry by scanning left to right, top to bottom, outputting a geometry point at each pixel. This output can be used to display image data on laser devices, oscilloscopes or similar devices using the [Laser CHOP](<./Laser_CHOP.md> "Laser CHOP"). This is different from the [Trace SOP](<./Trace_SOP.md> "Trace SOP") which only outputs contour shapes from an image. 

[![PythonIcon.png](./images/c/c2/PythonIcon.png)](</File:PythonIcon.png>)[rasterSOP_Class](</RasterSOP_Class> "RasterSOP Class")
...
Extra Information for the Raster SOP can be accessed via an [Info CHOP](<./Info_CHOP.md> "Info CHOP"). __
